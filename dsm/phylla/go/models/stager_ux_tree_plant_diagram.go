...
func (stager *Stager) treePlantDiagram(
	...
	*parentNodes = append(*parentNodes, plantDiagramNode)
	...
	rotatedCircleGridShape := plantDiagram.RotatedCircleGridShape
	if rotatedCircleGridShape != nil {
		node := &tree.Node{
			Name:            rotatedCircleGridShape.Name,
			IsNodeClickable: true,
		}
		node.OnClick = func(frontNode *tree.Node) {
			stager.probeForm.FillUpFormFromGongstruct(rotatedCircleGridShape, GetPointerToGongstructName[*CircleGridShape]())
			stager.stage.Commit()
		}
		btn := &tree.Button{
			Name:            "Hide",
			...
			ToolTipPosition: tree.Right,
			OnClick: func() {
				rotatedCircleGridShape.SetIsHidden(!rotatedCircleGridShape.GetIsHidden())
				stager.stage.Commit()
			},
		}
		if rotatedCircleGridShape.GetIsHidden() {
			btn.Icon = string(buttons.BUTTON_visibility)
			btn.Name = "Show"
			btn.ToolTipText = "Show on diagram"
		}
	...
	rotatedNextCircleShape := plantDiagram.RotatedNextCircleShape
	if rotatedNextCircleShape != nil {
		node := &tree.Node{
			Name:            rotatedNextCircleShape.Name,
			IsNodeClickable: true,
		}
		node.OnClick = func(frontNode *tree.Node) {
			stager.probeForm.FillUpFormFromGongstruct(rotatedNextCircleShape, GetPointerToGongstructName[*NextCircleShape]())
			stager.stage.Commit()
		}
		btn := &tree.Button{
			Name:            "Hide",
			...
			ToolTipPosition: tree.Right,
			OnClick: func() {
				rotatedNextCircleShape.SetIsHidden(!rotatedNextCircleShape.GetIsHidden())
				stager.stage.Commit()
			},
		}
		if rotatedNextCircleShape.GetIsHidden() {
			btn.Icon = string(buttons.BUTTON_visibility)
			btn.Name = "Show"
			btn.ToolTipText = "Show on diagram"
		}
