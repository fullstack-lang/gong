...
func (stageSet *StageSet) MarshallToString(packageName string) (res string, err error) {
	...
	if stageSet.Stage != nil {
		for _, musicabstract := range __gong__sortStageSetInstances(stageSet.Stage.MusicAbstracts, stageSet.Stage.MusicAbstract_stagedOrder) {
	...
	ast.Inspect(inFile, func(n ast.Node) bool {
		...
		case *ast.AssignStmt:
			...
			case "music":
				switch typeName {
				case "MusicAbstract":
					identifierMap[ident.Name] = __gong__stageSetInit(new(MusicAbstract), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				}
					}
				}
				return false
