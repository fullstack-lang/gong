...
func (stageSet *StageSet) MarshallToString(packageName string) (res string, err error) {
	...
	if stageSet.Stage != nil {
		for _, stoolabstract := range __gong__sortStageSetInstances(stageSet.Stage.StoolAbstracts, stageSet.Stage.StoolAbstract_stagedOrder) {
			...
			stoolabstractIdent := "__stool" + stoolabstract.GongGetIdentifier(stageSet.Stage)
			declarations.WriteString(fmt.Sprintf("\n\t%s := (&stool.StoolAbstract{Name: %s}).Stage(stageSet.Stage)", stoolabstractIdent, __gong__toRawStringLiteral(stoolabstract.Name)))
...
func _(stageSet *stool.StageSet) {
	...
	ast.Inspect(inFile, func(n ast.Node) bool {
		...
		case *ast.AssignStmt:
			...
			case "stool":
				switch typeName {
				case "StoolAbstract":
					identifierMap[ident.Name] = __gong__stageSetInit(new(StoolAbstract), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				}
					}
				}
				return false
