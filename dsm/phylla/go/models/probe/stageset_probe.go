...
func NewStageSetProbe(
	...
	metaPackageImports := []*doc_models.MetaPackageImport{
		{Alias: "ref_models", Path: "\"github.com/fullstack-lang/gong/dsm/phylla/go/models\""},
		{Alias: "ref_stool", Path: "\"github.com/fullstack-lang/gong/dsm/phylla/go/models/abstract/stool\""},
		{Alias: "ref_music", Path: "\"github.com/fullstack-lang/gong/dsm/phylla/go/models/abstract/music\""},
		{Alias: "ref_clock", Path: "\"github.com/fullstack-lang/gong/dsm/phylla/go/models/abstract/clock\""},
	}

	probe.docStager = prepare.PrepareStageSet(
		r,
		embeddedDiagrams,
		"github.com/fullstack-lang/gong/dsm/phylla/go:"+stageSet.GetProbeSplitStageName(),
		metaPackageImports,
		goModelsDir,
		goDiagramsDir,
		probe.diagramEditor,
...
func (probe *StageSetProbe) ComputeInstancesNb() map[string]int {
	...
	if probe.stageSet.StoolStage != nil {
		for k, v := range probe.stageSet.StoolStage.Map_GongStructName_InstancesNb {
			res[k] = v
			res["stool."+k] = v
		}
	}
	if probe.stageSet.MusicStage != nil {
		for k, v := range probe.stageSet.MusicStage.Map_GongStructName_InstancesNb {
			res[k] = v
			res["music."+k] = v
		}
	}
	if probe.stageSet.ClockStage != nil {
		for k, v := range probe.stageSet.ClockStage.Map_GongStructName_InstancesNb {
			res[k] = v
			res["clock."+k] = v
		}
	}
...
func (probe *StageSetProbe) ExportStage() {
	...
	if probe.fileName == "" {
		probe.fileName = "phylla-stageset.go"
	}
