...
func enforceShiftedLeftGrowthCurve2DRibbonHasShapes(
	...
	if ribbon == nil || stack == nil || plant.PerpendicularVectorGrid == nil || len(plant.PerpendicularVectorGrid.PerpendicularVectors) < 2 {
		if ribbon != nil && (len(ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes) > 0 || len(ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes) > 0) {
			for _, s := range ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes {
				s.Unstage(stage)
			}
			for _, s := range ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes {
				s.Unstage(stage)
			}
			ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes = nil
			ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes = nil
			return true
		}
		return false
	}
	...
	if len(ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes) != len(expectedStart) {
		for _, b := range ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes {
			b.Unstage(stage)
		}
		ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes = make([]*ShiftedLeftGrowthCurve2DRibbonStartShape, len(expectedStart))
		for i, exp := range expectedStart {
			b := new(ShiftedLeftGrowthCurve2DRibbonStartShape).Stage(stage)
			b.Name = exp.name
			b.BottomStartX = exp.bottomStartX
			b.BottomStartY = exp.bottomStartY
			b.BottomEndX = exp.bottomEndX
			...
			ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes[i] = b
		}
		needCommit = true
	} else {
		for i, exp := range expectedStart {
			b := ribbon.ShiftedLeftGrowthCurve2DRibbonStartShapes[i]
			if b.Name != exp.name ||
				b.BottomStartX != exp.bottomStartX || b.BottomStartY != exp.bottomStartY ||
				b.BottomEndX != exp.bottomEndX || b.BottomEndY != exp.bottomEndY ||
				b.TopStartX != exp.topStartX || b.TopStartY != exp.topStartY ||
	...
	if len(ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes) != len(expectedEnd) {
		for _, b := range ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes {
			b.Unstage(stage)
		}
		ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes = make([]*ShiftedLeftGrowthCurve2DRibbonEndShape, len(expectedEnd))
		for i, exp := range expectedEnd {
			b := new(ShiftedLeftGrowthCurve2DRibbonEndShape).Stage(stage)
			b.Name = exp.name
			b.BottomStartX = exp.bottomStartX
			b.BottomStartY = exp.bottomStartY
			b.BottomEndX = exp.bottomEndX
			...
			ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes[i] = b
		}
		needCommit = true
	} else {
		for i, exp := range expectedEnd {
			b := ribbon.ShiftedLeftGrowthCurve2DRibbonEndShapes[i]
			if b.Name != exp.name ||
				b.BottomStartX != exp.bottomStartX || b.BottomStartY != exp.bottomStartY ||
				b.BottomEndX != exp.bottomEndX || b.BottomEndY != exp.bottomEndY ||
				b.TopStartX != exp.topStartX || b.TopStartY != exp.topStartY ||
