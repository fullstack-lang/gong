...
import (
	...
	"github.com/fullstack-lang/gong/dsm/phylla/go/visual/threejs/cylinder"
...
func (u *Stool3DStageUpdater) ux_3d_stool(stager *models.Stager) {
	...
	params := cylinder.Cylinder3DParams{
		NamePrefix:            "Stool",
	...
	base := cylinder.RenderCylinder3DBase(stool3dStage, stager, plant, params)
	...
	if plant.StartArcShapeGrid != nil && len(plant.StartArcShapeGrid.StartArcShapes) > 0 {
		...
		if checkedDiagram != nil && !checkedDiagram.IsHiddenSeatBottomCurveShape {
		...
		if checkedDiagram != nil && !checkedDiagram.IsHiddenRotatedSeatBottomCurveShape {
		...
		if checkedDiagram == nil || !checkedDiagram.IsHiddenSeat3DShape {
		...
		if checkedDiagram != nil && !checkedDiagram.IsHiddenEyeSampledPoints3DShape {
			...
			cylinder.AddPointSpheres(stool3dStage, eyePoints, "magenta", canvas, "Stool Eye Sampled", 0, 0)
		}

		// 15. Render 3D Eye Corners Sampled Points
		if checkedDiagram != nil && !checkedDiagram.IsHiddenEyeCornersSampledPoints3DShape {
			...
			cylinder.AddPointSpheres(stool3dStage, cornerPoints, "cyan", canvas, "Stool Eye Corners Sampled", 0, 0)
		}
		...
		if checkedDiagram == nil || !checkedDiagram.IsHiddenEye3DShape {
		...
		if checkedDiagram == nil || !checkedDiagram.IsHiddenEyeVolume3DShape {
		...
		buildSeatAndLegsGeomLocal := func(geomName string, deltaTheta float64, deltaY float64) *threejs.BufferGeometry {
			return buildSeatAndLegsGeom(stool3dStage, geomName, deltaTheta, deltaY, rotSeatTopPoints, rotSeatBottomPoints, stoolTopHeight, seatBottomHeight, projSeatBottomEyePoints, projStoolBottomEyePoints, radialRepetitions)
		}

		// 20. 3D Seat and Legs (Union of seat and all legs)
		if checkedDiagram == nil || !checkedDiagram.IsHiddenSeatAndLegs3DShape {
		...
		if checkedDiagram != nil && !checkedDiagram.IsHiddenRotatedSeatAndLegs3DShape {
	...
	if checkedDiagram == nil || !checkedDiagram.IsHiddenTiledFloor3DShape {
		cylinder.AddFloorTiles(stool3dStage, canvas, globalR, floorMinY)
	}
