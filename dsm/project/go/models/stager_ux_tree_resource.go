...
func (stager *Stager) treeResourceinDiagram(diagram *Diagram, resource *Resource, parentNode *tree.Node) {
	...
	var resourceShape *ResourceShape
	shape, ok := diagram.map_Resource_ResourceShape[resource]
	if ok {
		resourceShape = shape
	}
	addLayoutButtons[*Resource, *ResourceShape](stager, resourceNode, resource, resourceShape, ok)
