// generated code - do not edit
package models

// insertion point
func (inst *DiagramHierarchy) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "Library":
		switch reverseField.Fieldname {
		case "Diagrams":
			if _library, ok := stage.Library_Diagrams_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	}
	return
}

func (inst *Library) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "Library":
		switch reverseField.Fieldname {
		case "SubLibraries":
			if _library, ok := stage.Library_SubLibraries_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	}
	return
}

func (inst *Milestone) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "MilestonesWhoseNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_MilestonesWhoseNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootMilestones":
			if _library, ok := stage.Library_RootMilestones_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	}
	return
}

func (inst *MilestoneShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "MilestoneShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_MilestoneShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *Note) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NotesWhoseNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_NotesWhoseNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	case "Library":
		switch reverseField.Fieldname {
		case "Notes":
			if _library, ok := stage.Library_Notes_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	}
	return
}

func (inst *NoteProductShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NoteProductShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_NoteProductShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *NoteResourceShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NoteResourceShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_NoteResourceShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *NoteShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Note_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_Note_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *NoteTaskShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NoteTaskShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_NoteTaskShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *Product) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ProductsWhoseNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_ProductsWhoseNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootProducts":
			if _library, ok := stage.Library_RootProducts_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	case "Note":
		switch reverseField.Fieldname {
		case "Products":
			if _note, ok := stage.Note_Products_reverseMap[inst]; ok {
				res = _note.Name
			}
		}
	case "Product":
		switch reverseField.Fieldname {
		case "SubProducts":
			if _product, ok := stage.Product_SubProducts_reverseMap[inst]; ok {
				res = _product.Name
			}
		}
	case "Task":
		switch reverseField.Fieldname {
		case "Inputs":
			if _task, ok := stage.Task_Inputs_reverseMap[inst]; ok {
				res = _task.Name
			}
		case "Outputs":
			if _task, ok := stage.Task_Outputs_reverseMap[inst]; ok {
				res = _task.Name
			}
		}
	}
	return
}

func (inst *ProductCompositionShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ProductComposition_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_ProductComposition_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *ProductShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Product_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_Product_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *Resource) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ResourcesWhoseNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_ResourcesWhoseNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootResources":
			if _library, ok := stage.Library_RootResources_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	case "Note":
		switch reverseField.Fieldname {
		case "Resources":
			if _note, ok := stage.Note_Resources_reverseMap[inst]; ok {
				res = _note.Name
			}
		}
	case "Resource":
		switch reverseField.Fieldname {
		case "SubResources":
			if _resource, ok := stage.Resource_SubResources_reverseMap[inst]; ok {
				res = _resource.Name
			}
		}
	}
	return
}

func (inst *ResourceCompositionShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ResourceComposition_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_ResourceComposition_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *ResourceShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Resource_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_Resource_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *ResourceTaskShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ResourceTaskShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_ResourceTaskShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *Task) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TasksWhoseNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TasksWhoseNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		case "TasksWhoseInputNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TasksWhoseInputNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		case "TasksWhoseOutputNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TasksWhoseOutputNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootTasks":
			if _library, ok := stage.Library_RootTasks_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	case "Note":
		switch reverseField.Fieldname {
		case "Tasks":
			if _note, ok := stage.Note_Tasks_reverseMap[inst]; ok {
				res = _note.Name
			}
		}
	case "Resource":
		switch reverseField.Fieldname {
		case "Tasks":
			if _resource, ok := stage.Resource_Tasks_reverseMap[inst]; ok {
				res = _resource.Name
			}
		}
	case "Task":
		switch reverseField.Fieldname {
		case "SubTasks":
			if _task, ok := stage.Task_SubTasks_reverseMap[inst]; ok {
				res = _task.Name
			}
		}
	case "TaskGroup":
		switch reverseField.Fieldname {
		case "Tasks":
			if _taskgroup, ok := stage.TaskGroup_Tasks_reverseMap[inst]; ok {
				res = _taskgroup.Name
			}
		}
	}
	return
}

func (inst *TaskCompositionShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskComposition_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TaskComposition_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *TaskGroup) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskGroupsWhoseNodeIsExpanded":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TaskGroupsWhoseNodeIsExpanded_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootTaskGroups":
			if _library, ok := stage.Library_RootTaskGroups_reverseMap[inst]; ok {
				res = _library.Name
			}
		}
	case "Milestone":
		switch reverseField.Fieldname {
		case "TaskGroupsToDisplay":
			if _milestone, ok := stage.Milestone_TaskGroupsToDisplay_reverseMap[inst]; ok {
				res = _milestone.Name
			}
		}
	}
	return
}

func (inst *TaskGroupShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskGroupShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TaskGroupShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *TaskInputShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskInputShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TaskInputShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *TaskOutputShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskOutputShapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_TaskOutputShapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

func (inst *TaskShape) GongGetReverseFieldOwnerName(stage *Stage, reverseField *ReverseField) (res string) {

	res = ""
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Task_Shapes":
			if _diagramhierarchy, ok := stage.DiagramHierarchy_Task_Shapes_reverseMap[inst]; ok {
				res = _diagramhierarchy.Name
			}
		}
	}
	return
}

// insertion point
func (inst *DiagramHierarchy) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "Library":
		switch reverseField.Fieldname {
		case "Diagrams":
			res = stage.Library_Diagrams_reverseMap[inst]
		}
	}
	return res
}

func (inst *Library) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "Library":
		switch reverseField.Fieldname {
		case "SubLibraries":
			res = stage.Library_SubLibraries_reverseMap[inst]
		}
	}
	return res
}

func (inst *Milestone) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "MilestonesWhoseNodeIsExpanded":
			res = stage.DiagramHierarchy_MilestonesWhoseNodeIsExpanded_reverseMap[inst]
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootMilestones":
			res = stage.Library_RootMilestones_reverseMap[inst]
		}
	}
	return res
}

func (inst *MilestoneShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "MilestoneShapes":
			res = stage.DiagramHierarchy_MilestoneShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *Note) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NotesWhoseNodeIsExpanded":
			res = stage.DiagramHierarchy_NotesWhoseNodeIsExpanded_reverseMap[inst]
		}
	case "Library":
		switch reverseField.Fieldname {
		case "Notes":
			res = stage.Library_Notes_reverseMap[inst]
		}
	}
	return res
}

func (inst *NoteProductShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NoteProductShapes":
			res = stage.DiagramHierarchy_NoteProductShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *NoteResourceShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NoteResourceShapes":
			res = stage.DiagramHierarchy_NoteResourceShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *NoteShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Note_Shapes":
			res = stage.DiagramHierarchy_Note_Shapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *NoteTaskShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "NoteTaskShapes":
			res = stage.DiagramHierarchy_NoteTaskShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *Product) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ProductsWhoseNodeIsExpanded":
			res = stage.DiagramHierarchy_ProductsWhoseNodeIsExpanded_reverseMap[inst]
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootProducts":
			res = stage.Library_RootProducts_reverseMap[inst]
		}
	case "Note":
		switch reverseField.Fieldname {
		case "Products":
			res = stage.Note_Products_reverseMap[inst]
		}
	case "Product":
		switch reverseField.Fieldname {
		case "SubProducts":
			res = stage.Product_SubProducts_reverseMap[inst]
		}
	case "Task":
		switch reverseField.Fieldname {
		case "Inputs":
			res = stage.Task_Inputs_reverseMap[inst]
		case "Outputs":
			res = stage.Task_Outputs_reverseMap[inst]
		}
	}
	return res
}

func (inst *ProductCompositionShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ProductComposition_Shapes":
			res = stage.DiagramHierarchy_ProductComposition_Shapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *ProductShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Product_Shapes":
			res = stage.DiagramHierarchy_Product_Shapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *Resource) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ResourcesWhoseNodeIsExpanded":
			res = stage.DiagramHierarchy_ResourcesWhoseNodeIsExpanded_reverseMap[inst]
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootResources":
			res = stage.Library_RootResources_reverseMap[inst]
		}
	case "Note":
		switch reverseField.Fieldname {
		case "Resources":
			res = stage.Note_Resources_reverseMap[inst]
		}
	case "Resource":
		switch reverseField.Fieldname {
		case "SubResources":
			res = stage.Resource_SubResources_reverseMap[inst]
		}
	}
	return res
}

func (inst *ResourceCompositionShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ResourceComposition_Shapes":
			res = stage.DiagramHierarchy_ResourceComposition_Shapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *ResourceShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Resource_Shapes":
			res = stage.DiagramHierarchy_Resource_Shapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *ResourceTaskShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "ResourceTaskShapes":
			res = stage.DiagramHierarchy_ResourceTaskShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *Task) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TasksWhoseNodeIsExpanded":
			res = stage.DiagramHierarchy_TasksWhoseNodeIsExpanded_reverseMap[inst]
		case "TasksWhoseInputNodeIsExpanded":
			res = stage.DiagramHierarchy_TasksWhoseInputNodeIsExpanded_reverseMap[inst]
		case "TasksWhoseOutputNodeIsExpanded":
			res = stage.DiagramHierarchy_TasksWhoseOutputNodeIsExpanded_reverseMap[inst]
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootTasks":
			res = stage.Library_RootTasks_reverseMap[inst]
		}
	case "Note":
		switch reverseField.Fieldname {
		case "Tasks":
			res = stage.Note_Tasks_reverseMap[inst]
		}
	case "Resource":
		switch reverseField.Fieldname {
		case "Tasks":
			res = stage.Resource_Tasks_reverseMap[inst]
		}
	case "Task":
		switch reverseField.Fieldname {
		case "SubTasks":
			res = stage.Task_SubTasks_reverseMap[inst]
		}
	case "TaskGroup":
		switch reverseField.Fieldname {
		case "Tasks":
			res = stage.TaskGroup_Tasks_reverseMap[inst]
		}
	}
	return res
}

func (inst *TaskCompositionShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskComposition_Shapes":
			res = stage.DiagramHierarchy_TaskComposition_Shapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *TaskGroup) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskGroupsWhoseNodeIsExpanded":
			res = stage.DiagramHierarchy_TaskGroupsWhoseNodeIsExpanded_reverseMap[inst]
		}
	case "Library":
		switch reverseField.Fieldname {
		case "RootTaskGroups":
			res = stage.Library_RootTaskGroups_reverseMap[inst]
		}
	case "Milestone":
		switch reverseField.Fieldname {
		case "TaskGroupsToDisplay":
			res = stage.Milestone_TaskGroupsToDisplay_reverseMap[inst]
		}
	}
	return res
}

func (inst *TaskGroupShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskGroupShapes":
			res = stage.DiagramHierarchy_TaskGroupShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *TaskInputShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskInputShapes":
			res = stage.DiagramHierarchy_TaskInputShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *TaskOutputShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "TaskOutputShapes":
			res = stage.DiagramHierarchy_TaskOutputShapes_reverseMap[inst]
		}
	}
	return res
}

func (inst *TaskShape) GongGetReverseFieldOwner(stage *Stage, reverseField *ReverseField) (res GongstructIF) {

	res = nil
	switch reverseField.GongstructName {
	// insertion point
	case "DiagramHierarchy":
		switch reverseField.Fieldname {
		case "Task_Shapes":
			res = stage.DiagramHierarchy_Task_Shapes_reverseMap[inst]
		}
	}
	return res
}
