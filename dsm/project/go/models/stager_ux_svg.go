...
func (stager *Stager) generateSvgObject(diagram *Diagram) *svg.SVG {
	...
	for _, taskShape := range diagram.Task_Shapes {
		...
		if taskShape.Task.IsWithCompletion {
			...
			if path := createCompletionRectAnchoredPath(taskShape.Task.Completion, distanceFromBorder, distanceFromBorder); path != nil {
	...
	for _, noteShape := range diagram.Note_Shapes {
		...
		rect := svgRect(
			...
			noteShape,
			layer)
		FormatNoteRect(stager, diagram, noteShape, rect)
...
func createCompletionRectAnchoredPath(completion CompletionEnum, xOffset, yOffset float64) *svg.RectAnchoredPath {
	...
	rectAnchoredPath.X_Offset = xOffset
	rectAnchoredPath.Y_Offset = yOffset
	rectAnchoredPath.RectAnchorType = svg.RECT_TOP_LEFT
