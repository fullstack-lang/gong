...
func (probe *Probe) ux_form() {
	...
	if formGroup != nil {
		...
		case *DiagramHierarchyFormCallback:
			if onSave.CreationMode {
				FillUpFormFromGongstructName(probe, "DiagramHierarchy", true)
			} else {
				FillUpFormFromGongstruct(onSave.diagramhierarchy, probe)
			}
		...
		case *NoteResourceShapeFormCallback:
			if onSave.CreationMode {
				FillUpFormFromGongstructName(probe, "NoteResourceShape", true)
			} else {
				FillUpFormFromGongstruct(onSave.noteresourceshape, probe)
			}
		...
		case *TaskGroupFormCallback:
			if onSave.CreationMode {
				FillUpFormFromGongstructName(probe, "TaskGroup", true)
			} else {
				FillUpFormFromGongstruct(onSave.taskgroup, probe)
			}
		case *TaskGroupShapeFormCallback:
			if onSave.CreationMode {
				FillUpFormFromGongstructName(probe, "TaskGroupShape", true)
			} else {
				FillUpFormFromGongstruct(onSave.taskgroupshape, probe)
			}
...
func FillUpFormFromGongstructName(
	...
	case "DiagramHierarchy":
		formGroup := (&form.FormGroup{
			Name:  FormName,
			Label: prefix + "DiagramHierarchy Form",
		}).Stage(formStage)
		...
		FillUpForm(diagramhierarchy, formGroup, probe)
	case "Library":
		formGroup := (&form.FormGroup{
			Name:  FormName,
