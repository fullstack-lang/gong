...
func NewDBWrapper(filename, table_prefix string, dst ...interface{}) orm.DBInterface {
...
// DBWrapper wraps *gorm.DB and implements the orm.DBInterface
type DBWrapper struct {
	db *gorm.DB
}

// Create inserts the value into the database and returns a new orm.DBInterface and an error if any.
func (dbw *DBWrapper) Create(instanceDB any) (orm.DBInterface, error) {
	tx := dbw.db.Create(instanceDB)
	return &DBWrapper{db: tx}, tx.Error
}

// Unscoped returns a new orm.DBInterface that includes soft-deleted records.
func (dbw *DBWrapper) Unscoped() (orm.DBInterface, error) {
	tx := dbw.db.Unscoped()
	return &DBWrapper{db: tx}, tx.Error
}

// Model specifies the model you would like to run db operations against.
func (dbw *DBWrapper) Model(instanceDB any) (orm.DBInterface, error) {
	tx := dbw.db.Model(instanceDB)
	return &DBWrapper{db: tx}, tx.Error
}

// Delete deletes value from the database.
func (dbw *DBWrapper) Delete(instanceDB any) (orm.DBInterface, error) {
	tx := dbw.db.Delete(instanceDB)
	return &DBWrapper{db: tx}, tx.Error
}

// Save updates value in the database.
func (dbw *DBWrapper) Save(instanceDB any) (orm.DBInterface, error) {
	tx := dbw.db.Save(instanceDB)
	return &DBWrapper{db: tx}, tx.Error
}

// Updates updates multiple columns.
func (dbw *DBWrapper) Updates(instanceDB any) (orm.DBInterface, error) {
	tx := dbw.db.Updates(instanceDB)
	return &DBWrapper{db: tx}, tx.Error
}

// Find finds records that match given conditions.
func (dbw *DBWrapper) Find(instanceDBs any) (orm.DBInterface, error) {
	tx := dbw.db.Find(instanceDBs)
	return &DBWrapper{db: tx}, tx.Error
}

// First returns the first record that matches the conditions.
func (dbw *DBWrapper) First(instanceDB any, conds ...any) (orm.DBInterface, error) {
