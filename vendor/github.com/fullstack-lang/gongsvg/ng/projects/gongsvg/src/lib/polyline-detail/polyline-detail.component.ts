...
export class PolylineDetailComponent implements OnInit {
	...
	state: PolylineDetailComponentState = PolylineDetailComponentState.CREATE_INSTANCE
			...
			frontRepo => {
				...
				switch (this.state) {
					...
					case PolylineDetailComponentState.UPDATE_INSTANCE:
						let polyline = frontRepo.Polylines.get(this.id)
						console.assert(polyline != undefined, "missing polyline with id:" + this.id)
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.ONE_MANY_ASSOCIATION_MODE) {
			...
			dialogData.ID = this.polyline.ID!
