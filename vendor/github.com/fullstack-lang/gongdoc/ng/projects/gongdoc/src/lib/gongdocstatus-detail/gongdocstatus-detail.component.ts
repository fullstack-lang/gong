...
enum GongdocStatusDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class GongdocStatusDetailComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		if (this.id == 0) {
			this.state = GongdocStatusDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = GongdocStatusDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getGongdocStatus(): void {
		...
		this.frontRepoService.pull().subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case GongdocStatusDetailComponentState.CREATE_INSTANCE:
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "GongdocStatus"
