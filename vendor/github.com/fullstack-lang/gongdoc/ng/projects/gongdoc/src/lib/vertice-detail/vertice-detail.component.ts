...
// VerticeDetailComponent is initilizaed from different routes
// VerticeDetailComponentState detail different cases 
enum VerticeDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class VerticeDetailComponent implements OnInit {
	...
	state: VerticeDetailComponentState = VerticeDetailComponentState.CREATE_INSTANCE
	...
	onChangedActivatedRoute(): void {
		...
		if (this.id == 0) {
			this.state = VerticeDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = VerticeDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getVertice(): void {
		...
		this.frontRepoService.pull(this.GONG__StackPath).subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case VerticeDetailComponentState.CREATE_INSTANCE:
						this.vertice = new (VerticeDB)
						break;
					case VerticeDetailComponentState.UPDATE_INSTANCE:
						let vertice = frontRepo.Vertices.get(this.id)
						console.assert(vertice != undefined, "missing vertice with id:" + this.id)
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "Vertice"
