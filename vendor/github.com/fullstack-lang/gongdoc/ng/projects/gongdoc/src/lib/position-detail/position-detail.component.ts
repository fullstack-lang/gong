...
export class PositionDetailComponent implements OnInit {
	...
	state: PositionDetailComponentState = PositionDetailComponentState.CREATE_INSTANCE
			...
			frontRepo => {
				...
				switch (this.state) {
					...
					case PositionDetailComponentState.UPDATE_INSTANCE:
						let position = frontRepo.Positions.get(this.id)
						console.assert(position != undefined, "missing position with id:" + this.id)
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.ONE_MANY_ASSOCIATION_MODE) {
			...
			dialogData.ID = this.position.ID!
