...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gongtree/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		...
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gongtree/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gongtree/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
