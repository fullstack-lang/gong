...
// TreeDetailComponent is initilizaed from different routes
// TreeDetailComponentState detail different cases 
enum TreeDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class TreeDetailComponent implements OnInit {
	...
	state: TreeDetailComponentState = TreeDetailComponentState.CREATE_INSTANCE
	...
	onChangedActivatedRoute(): void {
		...
		if (this.id == 0) {
			this.state = TreeDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = TreeDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getTree(): void {
		...
		this.frontRepoService.pull(this.GONG__StackPath).subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case TreeDetailComponentState.CREATE_INSTANCE:
						this.tree = new (TreeDB)
						break;
					case TreeDetailComponentState.UPDATE_INSTANCE:
						let tree = frontRepo.Trees.get(this.id)
						console.assert(tree != undefined, "missing tree with id:" + this.id)
						this.tree = tree!
						break;
	...
	fillUpNameIfEmpty(event: { value: { Name: string; }; }) {
		if (this.tree.Name == "") {
			this.tree.Name = event.value.Name
		}
	}
