...
func (backRepoGroup *BackRepoGroupStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, group *models.Group) (Error error) {
	...
	if groupDB, ok := backRepoGroup.Map_GroupDBID_GroupDB[idx]; ok {
		...
		// 1. reset
		groupDB.GroupPointersEncoding.Sliders = make([]int, 0)
		// 2. encode
		...
		// 1. reset
		groupDB.GroupPointersEncoding.Checkboxes = make([]int, 0)
		// 2. encode
...
func (groupDB *GroupDB) DecodePointers(backRepo *BackRepoStruct, group *models.Group) {
	...
	// This loop redeem group.Sliders in the stage from the encode in the back repo
	// It parses all SliderDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	group.Sliders = group.Sliders[:0]
	for _, _Sliderid := range groupDB.GroupPointersEncoding.Sliders {
		group.Sliders = append(group.Sliders, backRepo.BackRepoSlider.Map_SliderDBID_SliderPtr[uint(_Sliderid)])
	}

	// This loop redeem group.Checkboxes in the stage from the encode in the back repo
	// It parses all CheckboxDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	group.Checkboxes = group.Checkboxes[:0]
	for _, _Checkboxid := range groupDB.GroupPointersEncoding.Checkboxes {
