...
func (controller *Controller) UpdateCheckbox(c *gin.Context) {
	...
	if checkboxOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), checkboxOld, checkboxNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), checkboxOld, checkboxNew, mouseEvent)
