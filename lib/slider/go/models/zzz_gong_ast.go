				...
				if ok {
					...
					case "Group":
						...
						case "Sliders":
							...
							if start < len(instance.Sliders) && end <= len(instance.Sliders) && start < end {
								instance.Sliders = slices.Delete(instance.Sliders, start, end)
							}
						case "Checkboxes":
							instance := __gong__map_Group[identifier]
							if start < len(instance.Checkboxes) && end <= len(instance.Checkboxes) && start < end {
								instance.Checkboxes = slices.Delete(instance.Checkboxes, start, end)
							}
						}
					case "Layout":
						switch fieldName {
					...
					case "Group":
						...
						case "Sliders":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Slider[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Group[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Sliders) {
										instanceWhoseFieldIsAppended.Sliders = slices.Insert(instanceWhoseFieldIsAppended.Sliders, insertIndex, instanceToAppend)
						...
						case "Checkboxes":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Checkbox[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Group[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Checkboxes) {
										instanceWhoseFieldIsAppended.Checkboxes = slices.Insert(instanceWhoseFieldIsAppended.Checkboxes, insertIndex, instanceToAppend)
										insertIndex++ // Increment for subsequent elements in the same call
									}
								}
							}
