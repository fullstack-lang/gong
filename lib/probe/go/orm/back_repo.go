...
import (
	...
	"github.com/fullstack-lang/gong/lib/probe/go/orm/dbgorm"
	THIS IS REMOVED BY GONG COMPILER IF TARGET IS gorm */
...
type BackRepoStruct struct {
	...
	BackRepoProbe2 BackRepoProbe2Struct
...
func NewBackRepo(stage *models.Stage, filename string) (backRepo *BackRepoStruct) {
	...
	/* THIS IS REMOVED BY GONG COMPILER IF TARGET IS gorm
	db = dbgorm.NewDBWrapper(filename, "github_com_fullstack_lang_gong_lib_probe_go",
		&Probe2DB{},
	)
	THIS IS REMOVED BY GONG COMPILER IF TARGET IS gorm */
	...
	// insertion point for per struct back repo declarations
	backRepo.BackRepoProbe2 = BackRepoProbe2Struct{
		Map_Probe2DBID_Probe2Ptr: make(map[uint]*models.Probe2, 0),
		Map_Probe2DBID_Probe2DB:  make(map[uint]*Probe2DB, 0),
		Map_Probe2Ptr_Probe2DBID: make(map[*models.Probe2]uint, 0),
...
func (backRepo *BackRepoStruct) Commit(stage *models.Stage) {
	...
	backRepo.BackRepoProbe2.CommitPhaseOne(stage)

	// insertion point for per struct back repo phase two commit
	backRepo.BackRepoProbe2.CommitPhaseTwo(backRepo)
...
func (backRepo *BackRepoStruct) Checkout(stage *models.Stage) {
	...
	// insertion point for per struct back repo phase one commit
	backRepo.BackRepoProbe2.CheckoutPhaseOne()

	// insertion point for per struct back repo phase two commit
	backRepo.BackRepoProbe2.CheckoutPhaseTwo(backRepo)
}
...
func (backRepo *BackRepoStruct) Backup(stage *models.Stage, dirPath string) {
	...
	backRepo.BackRepoProbe2.Backup(dirPath)
}
...
func (backRepo *BackRepoStruct) BackupXL(stage *models.Stage, dirPath string) {
	...
	// insertion point for per struct backup
	backRepo.BackRepoProbe2.BackupXL(file)
...
func (backRepo *BackRepoStruct) Restore(stage *models.Stage, dirPath string) {
	...
	// insertion point for per struct backup
	backRepo.BackRepoProbe2.RestorePhaseOne(dirPath)
	...
	// insertion point for per struct backup
	backRepo.BackRepoProbe2.RestorePhaseTwo()
...
func (backRepo *BackRepoStruct) RestoreXL(stage *models.Stage, dirPath string) {
	...
	// insertion point for per struct backup
	backRepo.BackRepoProbe2.RestoreXLPhaseOne(file)
