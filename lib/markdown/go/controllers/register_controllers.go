...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	if backRepo == nil {
		message := "WebSocket Connect, Stack github.com/fullstack-lang/gong/lib/markdown/go, Unkown stack: \"" + stackPath + "\"\n"
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gong/lib/markdown/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("github.com/fullstack-lang/gong/lib/markdown/go:\n",
		...
		parts := strings.Split("github.com/fullstack-lang/gong/lib/markdown/go", "/") // Assuming goFilePath holds the path
		component := "unknown"
		if len(parts) > 2 {
