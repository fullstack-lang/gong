...
func registerControllers(r *gin.Engine) {
	v1 := r.Group("/api/github.com/fullstack-lang/gong/lib/ssg/go")
	{ // insertion point for registrations
		...
		v1.GET("/v1/pages", GetController().GetPages)
		v1.GET("/v1/pages/:id", GetController().GetPage)
		v1.POST("/v1/pages", GetController().PostPage)
		v1.PATCH("/v1/pages/:id", GetController().UpdatePage)
		v1.PUT("/v1/pages/:id", GetController().UpdatePage)
		v1.DELETE("/v1/pages/:id", GetController().DeletePage)
...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	// log.Println("Stack github.com/fullstack-lang/gong/lib/ssg/go, onWebSocketRequestForBackRepoContent")
	...
	log.Printf("Stack github.com/fullstack-lang/gong/lib/ssg/go: stack path: '%s', new ws index %d",
	...
	if backRepo == nil {
		message := "WebSocket Connect, Stack github.com/fullstack-lang/gong/lib/ssg/go, Unkown stack: \"" + stackPath + "\"\n"
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gong/lib/ssg/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("github.com/fullstack-lang/gong/lib/ssg/go:\n",
			"client no longer receiver web socket message, assuming it is no longer alive, closing websocket handler")
		fmt.Println(err)
		return
	} else {
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/ssg/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/ssg/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
