...
func (probe *Probe) ux_tree() {
	...
	for _, gongStruct := range sliceOfGongStructsSorted {
		...
		case "Chapter":
			...
			set := *probe.stageOfInterest.GetInstancesSet[*models.Chapter]()
			count := 0
		...
		case "Content":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.Content]()
			count := 0
		...
		case "DownloadableFile":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.DownloadableFile]()
			count := 0
			for _downloadablefile := range set {
				if count >= probe.GetMaxElementsNbPerGongStructNode() {
					nodeGongstruct.Children = append(nodeGongstruct.Children, &tree_models.Node{Name: "..."})
					break
				}
				count++
				nodeInstance := &tree_models.Node{
					Name:            _downloadablefile.GetName(),
					IsNodeClickable: true,
					OnClick: func(frontNode *tree_models.Node) {
						FillUpFormFromGongstruct(_downloadablefile, probe)
					},
				}
				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
		...
		case "JpgImage":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.JpgImage]()
			count := 0
		...
		case "Page":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.Page]()
			count := 0
			for _page := range set {
				if count >= probe.GetMaxElementsNbPerGongStructNode() {
					nodeGongstruct.Children = append(nodeGongstruct.Children, &tree_models.Node{Name: "..."})
				...
				nodeInstance := &tree_models.Node{
					...
					IsNodeClickable: true,
					OnClick: func(frontNode *tree_models.Node) {
						FillUpFormFromGongstruct(_page, probe)
					},
				}
				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
			...
			nodeGongstruct.OnClick = func(frontNode *tree_models.Node) {
				updateProbeTable[*models.Page](probe)
				// set color for node and reset all other nodes color
				for node := range *probe.treeStage.GetInstancesSet[*tree_models.Node]() {
					node.BackgroundColor = ""
				}
		...
		case "PngImage":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.PngImage]()
			count := 0
		...
		case "Section":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.Section]()
			count := 0
			for _section := range set {
				if count >= probe.GetMaxElementsNbPerGongStructNode() {
					nodeGongstruct.Children = append(nodeGongstruct.Children, &tree_models.Node{Name: "..."})
					break
				}
				...
				nodeInstance := &tree_models.Node{
					...
					IsNodeClickable: true,
					OnClick: func(frontNode *tree_models.Node) {
						FillUpFormFromGongstruct(_section, probe)
					},
				}
				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
			...
			nodeGongstruct.OnClick = func(frontNode *tree_models.Node) {
				updateProbeTable[*models.Section](probe)
				// set color for node and reset all other nodes color
				for node := range *probe.treeStage.GetInstancesSet[*tree_models.Node]() {
					node.BackgroundColor = ""
				}
		...
		case "SvgImage":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.SvgImage]()
			count := 0
