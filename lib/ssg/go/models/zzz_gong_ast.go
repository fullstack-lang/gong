			...
			if isSlicesDelete {
				...
				if ok {
					...
					case "Chapter":
						...
						case "Pages":
							instance := __gong__map_Chapter[identifier]
							if start < len(instance.Pages) && end <= len(instance.Pages) && start < end {
								instance.Pages = slices.Delete(instance.Pages, start, end)
							}
						}
					case "Content":
						switch fieldName {
						// insertion point for slices.Delete field code
						case "Chapters":
							instance := __gong__map_Content[identifier]
									...
									case "Page":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instancePage.StagePreserveOrder(stage, newOrder)
											}
										}
					...
					case "Chapter":
						...
						case "Pages":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Page[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Chapter[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Pages) {
										instanceWhoseFieldIsAppended.Pages = slices.Insert(instanceWhoseFieldIsAppended.Pages, insertIndex, instanceToAppend)
