...
export interface LinkConf {
    ...
    draggedSegmentPositionOnArrow: svg.PositionOnArrowType
    segments: Segment[] | undefined
    PointAtMouseDown: svg.Point | undefined

    // for change detection, we need to store start and end rect
    previousStart: svg.Rect | undefined
    previousEnd: svg.Rect | undefined
    map_Link_Segment: Map<svg.Link, Segment[]>
        ...
        if (linkConf.draggedSegmentNumber == linkConf.segments!.length - 1 && deltaY != 0) {
            ...
            if (newRatio < 0 || newRatio > 1) {
                ...
                if (nextStartRatio >= 0 && nextStartRatio <= 1) {
                    ...
                    link.EndOrientation = svg.OrientationType.ORIENTATION_VERTICAL
