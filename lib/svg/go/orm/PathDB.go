...
func (backRepoPath *BackRepoPathStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, path *models.Path) (Error error) {
	...
	if pathDB, ok := backRepoPath.Map_PathDBID_PathDB[idx]; ok {
		...
		// 1. reset
		pathDB.PathPointersEncoding.HoveringTrigger = make([]int, 0)
		// 2. encode
		for _, conditionAssocEnd := range path.HoveringTrigger {
			...
			pathDB.PathPointersEncoding.HoveringTrigger =
				append(pathDB.PathPointersEncoding.HoveringTrigger, int(conditionAssocEnd_DB.ID))
		}

		// 1. reset
		pathDB.PathPointersEncoding.DisplayConditions = make([]int, 0)
		// 2. encode
		for _, conditionAssocEnd := range path.DisplayConditions {
			conditionAssocEnd_DB :=
				backRepo.BackRepoCondition.GetConditionDBFromConditionPtr(conditionAssocEnd)
			...
			pathDB.PathPointersEncoding.DisplayConditions =
				append(pathDB.PathPointersEncoding.DisplayConditions, int(conditionAssocEnd_DB.ID))
		}
...
func (pathDB *PathDB) DecodePointers(backRepo *BackRepoStruct, path *models.Path) {
	...
	// This loop redeem path.HoveringTrigger in the stage from the encode in the back repo
	// It parses all ConditionDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	path.HoveringTrigger = path.HoveringTrigger[:0]
	for _, _Conditionid := range pathDB.PathPointersEncoding.HoveringTrigger {
		path.HoveringTrigger = append(path.HoveringTrigger, backRepo.BackRepoCondition.Map_ConditionDBID_ConditionPtr[uint(_Conditionid)])
	}

	// This loop redeem path.DisplayConditions in the stage from the encode in the back repo
	// It parses all ConditionDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	path.DisplayConditions = path.DisplayConditions[:0]
	for _, _Conditionid := range pathDB.PathPointersEncoding.DisplayConditions {
		path.DisplayConditions = append(path.DisplayConditions, backRepo.BackRepoCondition.Map_ConditionDBID_ConditionPtr[uint(_Conditionid)])
	}
