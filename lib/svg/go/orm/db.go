...
func (db *DBLite) Create(instanceDB any) (db.DBInterface, error) {
	if instanceDB == nil {
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, instanceDB cannot be nil")
	}
	...
	case *TextDB:
		...
		db.textDBs[v.ID] = v
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, unsupported type in Create")
	}
	return db, nil
}
...
func (db *DBLite) Delete(instanceDB any) (db.DBInterface, error) {
	...
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, unsupported type in Delete")
	}
	return db, nil
}
...
func (db *DBLite) Save(instanceDB any) (db.DBInterface, error) {
	...
	case *TextDB:
		...
		return db, nil
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, Save: unsupported type")
	}
}
	...
	case *AnimateDB:
		if existing, ok := db.animateDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db Animate github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *CircleDB:
	...
	case *LayerDB:
		if existing, ok := db.layerDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db Layer github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *LineDB:
		if existing, ok := db.lineDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Line github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *LinkDB:
	...
	case *LinkAnchoredTextDB:
		if existing, ok := db.linkanchoredtextDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db LinkAnchoredText github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *PathDB:
		if existing, ok := db.pathDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Path github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *PointDB:
		if existing, ok := db.pointDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Point github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *PolygoneDB:
		if existing, ok := db.polygoneDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Polygone github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *PolylineDB:
		if existing, ok := db.polylineDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Polyline github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *RectDB:
	...
	case *RectAnchoredTextDB:
		if existing, ok := db.rectanchoredtextDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db RectAnchoredText github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *RectLinkLinkDB:
		if existing, ok := db.rectlinklinkDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db RectLinkLink github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *SVGDB:
		if existing, ok := db.svgDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db SVG github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *SvgTextDB:
		if existing, ok := db.svgtextDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db SvgText github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	case *TextDB:
		if existing, ok := db.textDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Text github.com/fullstack-lang/gong/lib/svg/go, record not found")
		}
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, unsupported type in Updates")
	}
	return db, nil
}
...
func (db *DBLite) Find(instanceDBs any) (db.DBInterface, error) {
	...
	case *[]TextDB:
		*ptr = make([]TextDB, 0, len(db.textDBs))
		...
		return db, nil
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, Find: unsupported type")
	}
}
...
func (db *DBLite) First(instanceDB any, conds ...any) (db.DBInterface, error) {
	if len(conds) != 1 {
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, Do not process when conds is not a single parameter")
	}
	...
	case string:
		...
		if err != nil {
			return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, conds[0] is not a string number")
		}
	case uint64:
		i = cond
	case uint:
		i = uint64(cond)
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/svg/go, conds[0] is not a string or uint64")
	}
