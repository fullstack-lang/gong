...
func (backRepoLine *BackRepoLineStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, line *models.Line) (Error error) {
	...
	if lineDB, ok := backRepoLine.Map_LineDBID_LineDB[idx]; ok {
		...
		// 1. reset
		lineDB.LinePointersEncoding.HoveringTrigger = make([]int, 0)
		// 2. encode
		for _, conditionAssocEnd := range line.HoveringTrigger {
			...
			lineDB.LinePointersEncoding.HoveringTrigger =
				append(lineDB.LinePointersEncoding.HoveringTrigger, int(conditionAssocEnd_DB.ID))
		}

		// 1. reset
		lineDB.LinePointersEncoding.DisplayConditions = make([]int, 0)
		// 2. encode
		for _, conditionAssocEnd := range line.DisplayConditions {
			conditionAssocEnd_DB :=
				backRepo.BackRepoCondition.GetConditionDBFromConditionPtr(conditionAssocEnd)
			...
			lineDB.LinePointersEncoding.DisplayConditions =
				append(lineDB.LinePointersEncoding.DisplayConditions, int(conditionAssocEnd_DB.ID))
		}
...
func (lineDB *LineDB) DecodePointers(backRepo *BackRepoStruct, line *models.Line) {
	...
	// This loop redeem line.HoveringTrigger in the stage from the encode in the back repo
	// It parses all ConditionDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	line.HoveringTrigger = line.HoveringTrigger[:0]
	for _, _Conditionid := range lineDB.LinePointersEncoding.HoveringTrigger {
		line.HoveringTrigger = append(line.HoveringTrigger, backRepo.BackRepoCondition.Map_ConditionDBID_ConditionPtr[uint(_Conditionid)])
	}

	// This loop redeem line.DisplayConditions in the stage from the encode in the back repo
	// It parses all ConditionDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	line.DisplayConditions = line.DisplayConditions[:0]
	for _, _Conditionid := range lineDB.LinePointersEncoding.DisplayConditions {
		line.DisplayConditions = append(line.DisplayConditions, backRepo.BackRepoCondition.Map_ConditionDBID_ConditionPtr[uint(_Conditionid)])
	}
