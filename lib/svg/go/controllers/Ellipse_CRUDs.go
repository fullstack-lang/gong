...
func (controller *Controller) UpdateEllipse(c *gin.Context) {
	...
	if ellipseOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), ellipseOld, ellipseNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), ellipseOld, ellipseNew, mouseEvent)
