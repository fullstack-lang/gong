...
func (controller *Controller) UpdateRectAnchoredPath(c *gin.Context) {
	...
	if rectanchoredpathOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), rectanchoredpathOld, rectanchoredpathNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), rectanchoredpathOld, rectanchoredpathNew, mouseEvent)
