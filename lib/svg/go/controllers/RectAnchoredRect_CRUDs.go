...
func (controller *Controller) UpdateRectAnchoredRect(c *gin.Context) {
	...
	if rectanchoredrectOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), rectanchoredrectOld, rectanchoredrectNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), rectanchoredrectOld, rectanchoredrectNew, mouseEvent)
