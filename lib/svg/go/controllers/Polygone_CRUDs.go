...
func (controller *Controller) UpdatePolygone(c *gin.Context) {
	...
	if polygoneOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), polygoneOld, polygoneNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), polygoneOld, polygoneNew, mouseEvent)
