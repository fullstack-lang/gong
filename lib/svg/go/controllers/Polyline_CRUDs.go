...
func (controller *Controller) UpdatePolyline(c *gin.Context) {
	...
	if polylineOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), polylineOld, polylineNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), polylineOld, polylineNew, mouseEvent)
