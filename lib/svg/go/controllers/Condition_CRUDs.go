...
func (controller *Controller) UpdateCondition(c *gin.Context) {
	...
	if conditionOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), conditionOld, conditionNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), conditionOld, conditionNew, mouseEvent)
