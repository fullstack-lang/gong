...
func (controller *Controller) UpdateRectLinkLink(c *gin.Context) {
	...
	if rectlinklinkOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), rectlinklinkOld, rectlinklinkNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), rectlinklinkOld, rectlinklinkNew, mouseEvent)
