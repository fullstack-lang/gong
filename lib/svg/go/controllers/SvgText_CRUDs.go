...
func (controller *Controller) UpdateSvgText(c *gin.Context) {
	...
	if svgtextOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), svgtextOld, svgtextNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), svgtextOld, svgtextNew, mouseEvent)
