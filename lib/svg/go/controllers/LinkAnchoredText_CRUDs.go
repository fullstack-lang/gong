...
func (controller *Controller) UpdateLinkAnchoredText(c *gin.Context) {
	...
	if linkanchoredtextOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), linkanchoredtextOld, linkanchoredtextNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), linkanchoredtextOld, linkanchoredtextNew, mouseEvent)
