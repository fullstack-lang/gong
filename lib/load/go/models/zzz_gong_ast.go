						...
						case *ast.CompositeLit:
							...
							case *ast.SelectorExpr:
								...
								if Sel := slcExpr.Sel; Sel != nil {
									...
									case "FileToDownload":
										instanceFileToDownload := new(FileToDownload)
										instanceFileToDownload.Name = instanceName
										if !preserveOrder {
											instanceFileToDownload.Stage(stage)
										} else {
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												log.Println("UnmarshallGongstructStaging: Problem with parsing identifer", identifier)
												instanceFileToDownload.Stage(stage)
											} else {
												instanceFileToDownload.StagePreserveOrder(stage, newOrder)
											}
										}
										instance = any(instanceFileToDownload)
										__gong__map_FileToDownload[identifier] = instanceFileToDownload
									case "FileToUpload":
										instanceFileToUpload := new(FileToUpload)
										instanceFileToUpload.Name = instanceName
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceFileToUpload.StagePreserveOrder(stage, newOrder)
											}
										}
		...
		case *ast.BasicLit, *ast.UnaryExpr, *ast.CompositeLit:
			...
			case "FileToDownload":
				...
				case "Name":
					...
					__gong__map_FileToDownload[identifier].Name = fielValue
				case "Base64EncodedContent":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_FileToDownload[identifier].Base64EncodedContent = fielValue
				}
			case "FileToUpload":
				switch fieldName {
				// insertion point for field dependant code
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_FileToUpload[identifier].Name = fielValue
				case "Base64EncodedContent":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_FileToUpload[identifier].Base64EncodedContent = fielValue
				}
			case "Message":
				switch fieldName {
				// insertion point for field dependant code
