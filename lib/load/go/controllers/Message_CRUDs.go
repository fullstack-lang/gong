...
func (controller *Controller) UpdateMessage(c *gin.Context) {
	...
	if messageOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), messageOld, messageNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), messageOld, messageNew, mouseEvent)
