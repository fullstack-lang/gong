...
func (controller *Controller) UpdateFileToDownload(c *gin.Context) {
	...
	if filetodownloadOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), filetodownloadOld, filetodownloadNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), filetodownloadOld, filetodownloadNew, mouseEvent)
