...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	log.Printf(
		"%s github.com/fullstack-lang/gong/lib/load/go: Con: '%s', index %d",
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/load/go", stackPath, "WS client disconnected:", err)
	...
	log.Printf(
		"%s github.com/fullstack-lang/gong/lib/load/go: %03d: '%s', index %d",
		time.Now().Format("2006-01-02 15:04:05.000000"),
		refresh,
		stackPath, index,
	)
