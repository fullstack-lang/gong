...
func (controller *Controller) UpdateFileToUpload(c *gin.Context) {
	...
	if filetouploadOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), filetouploadOld, filetouploadNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), filetouploadOld, filetouploadNew, mouseEvent)
