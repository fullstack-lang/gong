...
func (controller *Controller) UpdateUpdateState(c *gin.Context) {
	...
	if updatestateOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), updatestateOld, updatestateNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), updatestateOld, updatestateNew, mouseEvent)
