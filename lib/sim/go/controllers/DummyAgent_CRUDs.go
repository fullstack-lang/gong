...
func (controller *Controller) UpdateDummyAgent(c *gin.Context) {
	...
	if dummyagentOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), dummyagentOld, dummyagentNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), dummyagentOld, dummyagentNew, mouseEvent)
