...
func (db *DBLite) Create(instanceDB any) (db.DBInterface, error) {
	if instanceDB == nil {
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, instanceDB cannot be nil")
	}
	...
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, unsupported type in Create")
	}
	return db, nil
}
...
func (db *DBLite) Delete(instanceDB any) (db.DBInterface, error) {
	...
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, unsupported type in Delete")
	}
	return db, nil
}
...
func (db *DBLite) Save(instanceDB any) (db.DBInterface, error) {
	...
	case *VerticeDB:
		...
		return db, nil
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, Save: unsupported type")
	}
}
	...
	case *ClassdiagramDB:
		if existing, ok := db.classdiagramDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db Classdiagram github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *DiagramPackageDB:
		if existing, ok := db.diagrampackageDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db DiagramPackage github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *FieldDB:
		if existing, ok := db.fieldDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Field github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *GongEnumShapeDB:
		if existing, ok := db.gongenumshapeDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db GongEnumShape github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *GongEnumValueEntryDB:
		if existing, ok := db.gongenumvalueentryDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db GongEnumValueEntry github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *GongStructShapeDB:
		if existing, ok := db.gongstructshapeDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db GongStructShape github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *LinkDB:
		if existing, ok := db.linkDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Link github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *NoteShapeDB:
		if existing, ok := db.noteshapeDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db NoteShape github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *NoteShapeLinkDB:
		if existing, ok := db.noteshapelinkDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db NoteShapeLink github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *PositionDB:
		if existing, ok := db.positionDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Position github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *UmlStateDB:
		if existing, ok := db.umlstateDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db UmlState github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *UmlscDB:
		if existing, ok := db.umlscDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Umlsc github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	case *VerticeDB:
		if existing, ok := db.verticeDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Vertice github.com/fullstack-lang/gong/lib/doc/go, record not found")
		}
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, unsupported type in Updates")
	}
	return db, nil
}
...
func (db *DBLite) Find(instanceDBs any) (db.DBInterface, error) {
	...
	case *[]VerticeDB:
		*ptr = make([]VerticeDB, 0, len(db.verticeDBs))
		...
		return db, nil
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, Find: unsupported type")
	}
}
...
func (db *DBLite) First(instanceDB any, conds ...any) (db.DBInterface, error) {
	if len(conds) != 1 {
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, Do not process when conds is not a single parameter")
	}
	...
	case string:
		...
		if err != nil {
			return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, conds[0] is not a string number")
		}
	case uint64:
		i = cond
	case uint:
		i = uint64(cond)
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, conds[0] is not a string or uint64")
	}
	...
	default:
		return nil, errors.New("github.com/fullstack-lang/gong/lib/doc/go, Unkown type")
	}
