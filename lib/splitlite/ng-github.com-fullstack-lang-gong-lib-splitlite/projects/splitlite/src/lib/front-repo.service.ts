...
export const StackType = "github.com/fullstack-lang/gong/lib/splitlite/go/models"
...
export class FrontRepoService {
	...
	public connectToWebSocket(Name: string): Observable<FrontRepo> {
		...
		// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; connectToWebSocket: started", Name)

		// Check if a connection for this name already exists
		if (this.webSocketConnections.has(Name)) {
			// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; connectToWebSocket: returning existing connection")
		...
		let basePath = `${protocol}//${host}/api/github.com/fullstack-lang/gong/lib/splitlite/go/v1/ws/stage`
		...
		const newConnection$ = new Observable<FrontRepo>(observer => {
			// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; connectToWebSocket: new Observable created")
			...
			const processData = (dataString: string) => {
				// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; connectToWebSocket: processData called")
			...
			const attemptConnection = (retries: number): void => {
				// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; attemptConnection: retries =", retries, "isOfflineMode =", isOfflineMode)

				// A. WASM OFFLINE MODE (Check if Go is ready)
				if ((window as any).openWasmSocket) {
					// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; attemptConnection: openWasmSocket exists, calling it");
					(window as any).openWasmSocket("github.com/fullstack-lang/gong/lib/splitlite/go", Name, processData);
					return;
				}

				// B. WAITING FOR WASM
				if (isOfflineMode && retries > 0) {
					// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; attemptConnection: WAITING FOR WASM. Retries left:", retries)
				...
				if (!isOfflineMode) {
					// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; attemptConnection: STANDARD SERVER MODE. url =", url)
					socket = new WebSocket(url)
					socket.onopen = (event) => {
						// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; WebSocket: onopen", event)
					}
					socket.onmessage = event => {
						// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; WebSocket: onmessage")
						processData(event.data)
					}
					socket.onerror = event => {
						console.error("github.com/fullstack-lang/gong/lib/splitlite/go WebSocket: onerror", event)
						observer.error(event)
					}
					socket.onclose = (event) => {
						// console.log("github.com/fullstack-lang/gong/lib/splitlite/go; WebSocket: onclose", event)
						observer.complete()
					}
				} else {
					console.error("github.com/fullstack-lang/gong/lib/splitlite/go, attemptConnection: Offline mode detected, but WASM backend failed to load.")
