...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	log.Printf("Stack github.com/fullstack-lang/gong/lib/button/go: stack path: '%s', new ws index %d",
	...
	if backRepo == nil {
		log.Panic("Stack github.com/fullstack-lang/gong/lib/button/go, Unkown stack", stackPath)
	}
	...
	if err != nil {
		...
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/button/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/button/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
