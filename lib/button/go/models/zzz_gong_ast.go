				...
				if ok {
					...
					case "Group":
						...
						case "Buttons":
							...
							if start < len(instance.Buttons) && end <= len(instance.Buttons) && start < end {
								instance.Buttons = slices.Delete(instance.Buttons, start, end)
							}
						}
					case "GroupToogle":
						switch fieldName {
						// insertion point for slices.Delete field code
						case "ButtonToggles":
							instance := __gong__map_GroupToogle[identifier]
							if start < len(instance.ButtonToggles) && end <= len(instance.ButtonToggles) && start < end {
								instance.ButtonToggles = slices.Delete(instance.ButtonToggles, start, end)
							}
						}
					case "Layout":
						switch fieldName {
						// insertion point for slices.Delete field code
						case "Groups":
							instance := __gong__map_Layout[identifier]
							if start < len(instance.Groups) && end <= len(instance.Groups) && start < end {
								instance.Groups = slices.Delete(instance.Groups, start, end)
							}
						case "GroupToogles":
							instance := __gong__map_Layout[identifier]
							if start < len(instance.GroupToogles) && end <= len(instance.GroupToogles) && start < end {
								instance.GroupToogles = slices.Delete(instance.GroupToogles, start, end)
							}
						}
					}
				}
									...
									case "ButtonToggle":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceButtonToggle.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "GroupToogle":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceGroupToogle.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Layout":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceLayout.StagePreserveOrder(stage, newOrder)
											}
										}
					...
					case "Group":
						...
						case "Buttons":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Button[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Group[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Buttons) {
										instanceWhoseFieldIsAppended.Buttons = slices.Insert(instanceWhoseFieldIsAppended.Buttons, insertIndex, instanceToAppend)
					...
					case "GroupToogle":
						...
						case "ButtonToggles":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_ButtonToggle[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_GroupToogle[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.ButtonToggles) {
										instanceWhoseFieldIsAppended.ButtonToggles = slices.Insert(instanceWhoseFieldIsAppended.ButtonToggles, insertIndex, instanceToAppend)
										insertIndex++ // Increment for subsequent elements in the same call
									}
								}
							}
					...
					case "Layout":
						...
						case "GroupToogles":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_GroupToogle[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Layout[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.GroupToogles) {
										instanceWhoseFieldIsAppended.GroupToogles = slices.Insert(instanceWhoseFieldIsAppended.GroupToogles, insertIndex, instanceToAppend)
										insertIndex++ // Increment for subsequent elements in the same call
									}
								}
							}
		...
		case *ast.BasicLit, *ast.UnaryExpr, *ast.CompositeLit:
			...
			case "Button":
				...
				case "Label":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_Button[identifier].Label = fielValue
				case "Icon":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
