...
func (stageSet *StageSet) MarshallToString(packageName string) (res string, err error) {
	...
	if stageSet.Stage != nil {
		for _, buttontoggle := range __gong__sortStageSetInstances(stageSet.Stage.ButtonToggles, stageSet.Stage.ButtonToggle_stagedOrder) {
			if lastStageDecl != "Stage" {
				if declarations.Len() > 0 {
					declarations.WriteString("\n")
				}
	...
	if stageSet.Stage != nil {
		for _, grouptoogle := range __gong__sortStageSetInstances(stageSet.Stage.GroupToogles, stageSet.Stage.GroupToogle_stagedOrder) {
			if lastStageDecl != "Stage" {
				if declarations.Len() > 0 {
					declarations.WriteString("\n")
				}
	...
	if stageSet.Stage != nil {
		for _, layout := range __gong__sortStageSetInstances(stageSet.Stage.Layouts, stageSet.Stage.Layout_stagedOrder) {
			if lastStageDecl != "Stage" {
				if declarations.Len() > 0 {
					declarations.WriteString("\n")
				}
	...
	ast.Inspect(inFile, func(n ast.Node) bool {
		...
		case *ast.AssignStmt:
			...
			case "models":
				...
				case "ButtonToggle":
					identifierMap[ident.Name] = __gong__stageSetInit(new(ButtonToggle), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				case "Group":
					identifierMap[ident.Name] = __gong__stageSetInit(new(Group), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				case "GroupToogle":
					identifierMap[ident.Name] = __gong__stageSetInit(new(GroupToogle), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				case "Layout":
					identifierMap[ident.Name] = __gong__stageSetInit(new(Layout), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				}
					}
				}
				return false
			...
			if node.Tok == token.ASSIGN {
				...
				case *GroupToogle:
					...
					case "Percentage":
						inst.Percentage = GongExtractFloat(rhs)
					case "ButtonToggles":
						__gong__assignSliceOfPointers(&inst.ButtonToggles, rhs, identifierMap)
				...
				case *Layout:
					...
					case "GroupToogles":
						__gong__assignSliceOfPointers(&inst.GroupToogles, rhs, identifierMap)
					}
							}
						}
					}
