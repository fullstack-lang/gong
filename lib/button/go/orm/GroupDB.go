...
func (backRepoGroup *BackRepoGroupStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, group *models.Group) (Error error) {
	...
	if groupDB, ok := backRepoGroup.Map_GroupDBID_GroupDB[idx]; ok {
		...
		// 1. reset
		groupDB.GroupPointersEncoding.Buttons = make([]int, 0)
		// 2. encode
...
func (groupDB *GroupDB) DecodePointers(backRepo *BackRepoStruct, group *models.Group) {
	...
	// This loop redeem group.Buttons in the stage from the encode in the back repo
	...
	for _, _Buttonid := range groupDB.GroupPointersEncoding.Buttons {
		group.Buttons = append(group.Buttons, backRepo.BackRepoButton.Map_ButtonDBID_ButtonPtr[uint(_Buttonid)])
	}
