...
func (controller *Controller) UpdateFavIcon(c *gin.Context) {
	...
	if faviconOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), faviconOld, faviconNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), faviconOld, faviconNew, mouseEvent)
