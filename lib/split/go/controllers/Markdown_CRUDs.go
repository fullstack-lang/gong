...
func (controller *Controller) UpdateMarkdown(c *gin.Context) {
	...
	if markdownOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), markdownOld, markdownNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), markdownOld, markdownNew, mouseEvent)
