...
func (controller *Controller) UpdateAsSplitArea(c *gin.Context) {
	...
	if assplitareaOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), assplitareaOld, assplitareaNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), assplitareaOld, assplitareaNew, mouseEvent)
