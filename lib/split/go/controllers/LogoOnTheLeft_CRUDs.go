...
func (controller *Controller) UpdateLogoOnTheLeft(c *gin.Context) {
	...
	if logoontheleftOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), logoontheleftOld, logoontheleftNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), logoontheleftOld, logoontheleftNew, mouseEvent)
