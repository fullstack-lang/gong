...
func (controller *Controller) UpdateLogoOnTheRight(c *gin.Context) {
	...
	if logoontherightOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), logoontherightOld, logoontherightNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), logoontherightOld, logoontherightNew, mouseEvent)
