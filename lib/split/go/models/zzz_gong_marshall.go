...
func (stage *Stage) MarshallToString(modelsPackageName, packageName string) (res string, err error) {
	...
	sort.Slice(assplitOrdered[:], func(i, j int) bool {
		...
		asspliti_order, oki := stage.AsSplit_stagedOrder[asspliti]
		assplitj_order, okj := stage.AsSplit_stagedOrder[assplitj]
		if !oki || !okj {
	...
	sort.Slice(assplitareaOrdered[:], func(i, j int) bool {
		...
		assplitareai_order, oki := stage.AsSplitArea_stagedOrder[assplitareai]
		assplitareaj_order, okj := stage.AsSplitArea_stagedOrder[assplitareaj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(faviconOrdered[:], func(i, j int) bool {
		...
		faviconi_order, oki := stage.FavIcon_stagedOrder[faviconi]
		faviconj_order, okj := stage.FavIcon_stagedOrder[faviconj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(formOrdered[:], func(i, j int) bool {
		formi := formOrdered[i]
		formj := formOrdered[j]
		formi_order, oki := stage.Form_stagedOrder[formi]
		formj_order, okj := stage.Form_stagedOrder[formj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(loadOrdered[:], func(i, j int) bool {
		loadi := loadOrdered[i]
		loadj := loadOrdered[j]
		loadi_order, oki := stage.Load_stagedOrder[loadi]
		loadj_order, okj := stage.Load_stagedOrder[loadj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(logoontheleftOrdered[:], func(i, j int) bool {
		...
		logoonthelefti_order, oki := stage.LogoOnTheLeft_stagedOrder[logoonthelefti]
		logoontheleftj_order, okj := stage.LogoOnTheLeft_stagedOrder[logoontheleftj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(logoontherightOrdered[:], func(i, j int) bool {
		...
		logoontherighti_order, oki := stage.LogoOnTheRight_stagedOrder[logoontherighti]
		logoontherightj_order, okj := stage.LogoOnTheRight_stagedOrder[logoontherightj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(markdownOrdered[:], func(i, j int) bool {
		...
		markdowni_order, oki := stage.Markdown_stagedOrder[markdowni]
		markdownj_order, okj := stage.Markdown_stagedOrder[markdownj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(splitOrdered[:], func(i, j int) bool {
		...
		spliti_order, oki := stage.Split_stagedOrder[spliti]
		splitj_order, okj := stage.Split_stagedOrder[splitj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(titleOrdered[:], func(i, j int) bool {
		...
		titlei_order, oki := stage.Title_stagedOrder[titlei]
		titlej_order, okj := stage.Title_stagedOrder[titlej]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(toneOrdered[:], func(i, j int) bool {
		tonei := toneOrdered[i]
		tonej := toneOrdered[j]
		tonei_order, oki := stage.Tone_stagedOrder[tonei]
		tonej_order, okj := stage.Tone_stagedOrder[tonej]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(viewOrdered[:], func(i, j int) bool {
		viewi := viewOrdered[i]
		viewj := viewOrdered[j]
		viewi_order, oki := stage.View_stagedOrder[viewi]
		viewj_order, okj := stage.View_stagedOrder[viewj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
	...
	sort.Slice(xlsxOrdered[:], func(i, j int) bool {
		xlsxi := xlsxOrdered[i]
		xlsxj := xlsxOrdered[j]
		xlsxi_order, oki := stage.Xlsx_stagedOrder[xlsxi]
		xlsxj_order, okj := stage.Xlsx_stagedOrder[xlsxj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
...
func (assplit *AsSplit) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(assplit.Name))
	case "Direction":
...
func (assplitarea *AsSplitArea) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(assplitarea.Name))
	...
	case "DivStyle":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(assplitarea.DivStyle))
...
func (button *Button) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "StackName":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(button.StackName))
...
func (cursor *Cursor) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "StackName":
		...
		res = strings.ReplaceAll(res, "{{Identifier}}", cursor.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(cursor.StackName))
	case "Style":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", cursor.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Style")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(cursor.Style))
...
func (favicon *FavIcon) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(favicon.Name))
	case "SVG":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", favicon.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "SVG")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(favicon.SVG))
...
func (form *Form) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(form.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", form.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(form.StackName))
...
func (load *Load) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(load.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", load.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(load.StackName))
...
func (logoontheleft *LogoOnTheLeft) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(logoontheleft.Name))
	case "Width":
		res = NumberInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", logoontheleft.GongGetIdentifier(stage))
	...
	case "SVG":
		...
		res = strings.ReplaceAll(res, "{{Identifier}}", logoontheleft.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "SVG")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(logoontheleft.SVG))
...
func (logoontheright *LogoOnTheRight) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(logoontheright.Name))
	case "Width":
		res = NumberInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", logoontheright.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Width")
	...
	case "SVG":
		...
		res = strings.ReplaceAll(res, "{{Identifier}}", logoontheright.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "SVG")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(logoontheright.SVG))
...
func (markdown *Markdown) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(markdown.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", markdown.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(markdown.StackName))
...
func (slider *Slider) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(slider.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", slider.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(slider.StackName))
...
func (split *Split) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(split.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", split.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(split.StackName))
...
func (svg *Svg) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "StackName":
		...
		res = strings.ReplaceAll(res, "{{Identifier}}", svg.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(svg.StackName))
	case "Style":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", svg.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Style")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(svg.Style))
...
func (table *Table) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "StackName":
		...
		res = strings.ReplaceAll(res, "{{Identifier}}", table.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(table.StackName))
...
func (title *Title) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(title.Name))
...
func (tone *Tone) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(tone.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", tone.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(tone.StackName))
...
func (tree *Tree) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "StackName":
		...
		res = strings.ReplaceAll(res, "{{Identifier}}", tree.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(tree.StackName))
...
func (view *View) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(view.Name))
	case "ShowViewName":
		res = NumberInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", view.GongGetIdentifier(stage))
...
func (xlsx *Xlsx) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(xlsx.Name))
	case "StackName":
		res = StringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", xlsx.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "StackName")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(xlsx.StackName))
