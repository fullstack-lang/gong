			...
			if isSlicesDelete {
				...
				if ok {
					...
					case "AsSplit":
						...
						case "AsSplitAreas":
							instance := __gong__map_AsSplit[identifier]
							if start < len(instance.AsSplitAreas) && end <= len(instance.AsSplitAreas) && start < end {
								instance.AsSplitAreas = slices.Delete(instance.AsSplitAreas, start, end)
							}
						}
					case "AsSplitArea":
						switch fieldName {
					...
					case "View":
						...
						case "RootAsSplitAreas":
							instance := __gong__map_View[identifier]
							if start < len(instance.RootAsSplitAreas) && end <= len(instance.RootAsSplitAreas) && start < end {
								instance.RootAsSplitAreas = slices.Delete(instance.RootAsSplitAreas, start, end)
							}
						}
					case "Xlsx":
						switch fieldName {
									...
									case "AsSplit":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceAsSplit.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "AsSplitArea":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceAsSplitArea.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Button":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceButton.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Cursor":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceCursor.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "FavIcon":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceFavIcon.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Form":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceForm.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Load":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceLoad.StagePreserveOrder(stage, newOrder)
											}
										}
										instance = any(instanceLoad)
										__gong__map_Load[identifier] = instanceLoad
									case "LogoOnTheLeft":
										instanceLogoOnTheLeft := new(LogoOnTheLeft)
										instanceLogoOnTheLeft.Name = instanceName
										if !preserveOrder {
											instanceLogoOnTheLeft.Stage(stage)
										} else {
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												log.Println("UnmarshallGongstructStaging: Problem with parsing identifer", identifier)
												instanceLogoOnTheLeft.Stage(stage)
											} else {
												instanceLogoOnTheLeft.StagePreserveOrder(stage, newOrder)
											}
										}
										instance = any(instanceLogoOnTheLeft)
										__gong__map_LogoOnTheLeft[identifier] = instanceLogoOnTheLeft
									case "LogoOnTheRight":
										instanceLogoOnTheRight := new(LogoOnTheRight)
										instanceLogoOnTheRight.Name = instanceName
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceLogoOnTheRight.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Markdown":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceMarkdown.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Slider":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceSlider.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Split":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceSplit.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Title":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceTitle.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Tone":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceTone.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Tree":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceTree.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "View":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceView.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Xlsx":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceXlsx.StagePreserveOrder(stage, newOrder)
											}
										}
					...
					case "AsSplit":
						...
						case "AsSplitAreas":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_AsSplitArea[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_AsSplit[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.AsSplitAreas) {
										instanceWhoseFieldIsAppended.AsSplitAreas = slices.Insert(instanceWhoseFieldIsAppended.AsSplitAreas, insertIndex, instanceToAppend)
					...
					case "View":
						...
						case "RootAsSplitAreas":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_AsSplitArea[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_View[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.RootAsSplitAreas) {
										instanceWhoseFieldIsAppended.RootAsSplitAreas = slices.Insert(instanceWhoseFieldIsAppended.RootAsSplitAreas, insertIndex, instanceToAppend)
										insertIndex++ // Increment for subsequent elements in the same call
									}
								}
							}
		...
		case *ast.BasicLit, *ast.UnaryExpr, *ast.CompositeLit:
			...
			case "LogoOnTheLeft":
				...
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_LogoOnTheLeft[identifier].Name = fielValue
				case "Width":
					...
					__gong__map_LogoOnTheLeft[identifier].Width = int(exprSign) * int(fielValue)
				case "Height":
					// convert string to int
					fielValue, err := strconv.ParseInt(basicLit.Value, 10, 64)
					if err != nil {
					...
					__gong__map_LogoOnTheLeft[identifier].Height = int(exprSign) * int(fielValue)
				case "SVG":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_LogoOnTheLeft[identifier].SVG = fielValue
				}
			case "LogoOnTheRight":
				switch fieldName {
				// insertion point for field dependant code
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_LogoOnTheRight[identifier].Name = fielValue
				case "Width":
					// convert string to int
					fielValue, err := strconv.ParseInt(basicLit.Value, 10, 64)
					if err != nil {
					...
					__gong__map_LogoOnTheRight[identifier].Width = int(exprSign) * int(fielValue)
				case "Height":
					// convert string to int
					fielValue, err := strconv.ParseInt(basicLit.Value, 10, 64)
					if err != nil {
					...
					__gong__map_LogoOnTheRight[identifier].Height = int(exprSign) * int(fielValue)
				case "SVG":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
		...
		case *ast.Ident:
			...
			case "View":
				...
				case "IsSelectedView":
					...
					__gong__map_View[identifier].IsSelectedView = fielValue
				}
			case "Xlsx":
				switch fieldName {
				// insertion point for field dependant code
