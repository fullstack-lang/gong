				...
				if ok {
					...
					case "Note":
						...
						case "Frequencies":
							...
							if start < len(instance.Frequencies) && end <= len(instance.Frequencies) && start < end {
								instance.Frequencies = slices.Delete(instance.Frequencies, start, end)
							}
						}
					case "Player":
						switch fieldName {
									...
									case "Freqency":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceFreqency.StagePreserveOrder(stage, newOrder)
											}
										}
									...
									case "Player":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instancePlayer.StagePreserveOrder(stage, newOrder)
											}
										}
					...
					case "Note":
						...
						case "Frequencies":
							...
							if isAppend && argNb > 0 {
								...
								if instanceToAppend, ok := __gong__map_Freqency[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Note[identifier]
									instanceWhoseFieldIsAppended.Frequencies = append(instanceWhoseFieldIsAppended.Frequencies, instanceToAppend)
								}
							}
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Freqency[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Note[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Frequencies) {
										instanceWhoseFieldIsAppended.Frequencies = slices.Insert(instanceWhoseFieldIsAppended.Frequencies, insertIndex, instanceToAppend)
		...
		case *ast.BasicLit, *ast.UnaryExpr, *ast.CompositeLit:
			...
			case "Freqency":
				...
				case "Name":
					...
					__gong__map_Freqency[identifier].Name = fielValue
				}
			case "Note":
				switch fieldName {
				// insertion point for field dependant code
				...
				case "Start":
					...
					__gong__map_Note[identifier].Start = exprSign * fielValue
				case "Duration":
					// convert string to float64
					fielValue, err := strconv.ParseFloat(basicLit.Value, 64)
					if err != nil {
					...
					__gong__map_Note[identifier].Duration = exprSign * fielValue
				case "Velocity":
					// convert string to float64
					fielValue, err := strconv.ParseFloat(basicLit.Value, 64)
					if err != nil {
