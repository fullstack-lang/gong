...
func (stageSet *StageSet) MarshallToString(packageName string) (res string, err error) {
	...
	if stageSet.Stage != nil {
		for _, freqency := range __gong__sortStageSetInstances(stageSet.Stage.Freqencys, stageSet.Stage.Freqency_stagedOrder) {
			...
			freqencyIdent := "__models" + freqency.GongGetIdentifier(stageSet.Stage)
			declarations.WriteString(fmt.Sprintf("\n\t%s := (&models.Freqency{Name: %s}).Stage(stageSet.Stage)", freqencyIdent, __gong__toRawStringLiteral(freqency.Name)))
...
func (stageSet *StageSet) ParseAstFileFromAst(inFile *ast.File, fset *token.FileSet, preserveOrder bool) error {
	...
	for _, imp := range inFile.Imports {
		...
		switch p {
		case "github.com/fullstack-lang/gong/lib/tone/go/models":
	...
	ast.Inspect(inFile, func(n ast.Node) bool {
		...
		case *ast.AssignStmt:
			...
			case "models":
				switch typeName {
				case "Freqency":
					identifierMap[ident.Name] = __gong__stageSetInit(new(Freqency), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				case "Note":
			...
			if node.Tok == token.ASSIGN {
				...
				case *Note:
					...
					case "Name":
						inst.Name = GongExtractString(rhs)
					case "Frequencies":
						__gong__assignSliceOfPointers(&inst.Frequencies, rhs, identifierMap)
					case "Start":
