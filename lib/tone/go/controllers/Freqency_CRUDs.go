...
func (controller *Controller) UpdateFreqency(c *gin.Context) {
	...
	if freqencyOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), freqencyOld, freqencyNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), freqencyOld, freqencyNew, mouseEvent)
