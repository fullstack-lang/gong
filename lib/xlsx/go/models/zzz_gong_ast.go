...
var __gong__map_DisplaySelection = make(map[string]*DisplaySelection)
var __gong__map_XLCell = make(map[string]*XLCell)
var __gong__map_XLFile = make(map[string]*XLFile)
var __gong__map_XLRow = make(map[string]*XLRow)
var __gong__map_XLSheet = make(map[string]*XLSheet)
			...
			if isSlicesDelete {
				...
				if ok {
					...
					case "XLFile":
						...
						// insertion point for slices.Delete field code
						case "Sheets":
							instance := __gong__map_XLFile[identifier]
							if start < len(instance.Sheets) && end <= len(instance.Sheets) && start < end {
								instance.Sheets = slices.Delete(instance.Sheets, start, end)
							}
						}
					case "XLRow":
						switch fieldName {
						// insertion point for slices.Delete field code
						case "Cells":
							instance := __gong__map_XLRow[identifier]
					...
					case "XLSheet":
						...
						// insertion point for slices.Delete field code
						case "Rows":
							instance := __gong__map_XLSheet[identifier]
							if start < len(instance.Rows) && end <= len(instance.Rows) && start < end {
								instance.Rows = slices.Delete(instance.Rows, start, end)
							}
						case "SheetCells":
							instance := __gong__map_XLSheet[identifier]
							if start < len(instance.SheetCells) && end <= len(instance.SheetCells) && start < end {
								instance.SheetCells = slices.Delete(instance.SheetCells, start, end)
							}
						}
					}
				}
					...
					case "XLFile":
						...
						case "Sheets":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_XLSheet[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_XLFile[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Sheets) {
										instanceWhoseFieldIsAppended.Sheets = slices.Insert(instanceWhoseFieldIsAppended.Sheets, insertIndex, instanceToAppend)
					...
					case "XLSheet":
						...
						case "SheetCells":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_XLCell[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_XLSheet[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.SheetCells) {
										instanceWhoseFieldIsAppended.SheetCells = slices.Insert(instanceWhoseFieldIsAppended.SheetCells, insertIndex, instanceToAppend)
										insertIndex++ // Increment for subsequent elements in the same call
									}
								}
							}
		...
		case *ast.BasicLit, *ast.UnaryExpr, *ast.CompositeLit:
			...
			case "DisplaySelection":
				...
				case "Name":
					...
					__gong__map_DisplaySelection[identifier].Name = fielValue
				}
			case "XLCell":
				switch fieldName {
				// insertion point for field dependant code
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_XLCell[identifier].Name = fielValue
				case "X":
			...
			case "XLFile":
				...
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_XLFile[identifier].Name = fielValue
				case "NbSheets":
					// convert string to int
					fielValue, err := strconv.ParseInt(basicLit.Value, 10, 64)
					if err != nil {
			...
			case "XLRow":
				...
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_XLRow[identifier].Name = fielValue
				case "RowIndex":
					// convert string to int
					fielValue, err := strconv.ParseInt(basicLit.Value, 10, 64)
					if err != nil {
			...
			case "XLSheet":
				...
				case "Name":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_XLSheet[identifier].Name = fielValue
				case "MaxRow":
					// convert string to int
					fielValue, err := strconv.ParseInt(basicLit.Value, 10, 64)
					if err != nil {
		...
		case *ast.Ident:
			...
			case "DisplaySelection":
				...
				case "XLFile":
					...
					__gong__map_DisplaySelection[identifier].XLFile = __gong__map_XLFile[targetIdentifier]
				case "XLSheet":
					targetIdentifier := ident.Name
					__gong__map_DisplaySelection[identifier].XLSheet = __gong__map_XLSheet[targetIdentifier]
				}
			case "XLCell":
				switch fieldName {
				// insertion point for field dependant code
