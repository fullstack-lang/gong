...
func (controller *Controller) UpdateXLSheet(c *gin.Context) {
	...
	if xlsheetOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), xlsheetOld, xlsheetNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), xlsheetOld, xlsheetNew, mouseEvent)
