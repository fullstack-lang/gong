				...
				if ok {
					...
					case "Node":
						...
						case "Children":
							...
							if start < len(instance.Children) && end <= len(instance.Children) && start < end {
								instance.Children = slices.Delete(instance.Children, start, end)
							}
						case "Buttons":
							instance := __gong__map_Node[identifier]
					...
					case "Tree":
						...
						// insertion point for slices.Delete field code
						case "RootNodes":
							instance := __gong__map_Tree[identifier]
							if start < len(instance.RootNodes) && end <= len(instance.RootNodes) && start < end {
								instance.RootNodes = slices.Delete(instance.RootNodes, start, end)
							}
						}
					}
				}
									...
									case "SVGIcon":
										...
										if !preserveOrder {
											...
											if newOrder, err := ExtractMiddleUint(identifier); err != nil {
												...
												instanceSVGIcon.StagePreserveOrder(stage, newOrder)
											}
										}
					...
					case "Node":
						...
						case "Children":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Node[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Node[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.Children) {
										instanceWhoseFieldIsAppended.Children = slices.Insert(instanceWhoseFieldIsAppended.Children, insertIndex, instanceToAppend)
					...
					case "Tree":
						...
						case "RootNodes":
							...
							if isSlicesInsert && argNb > 1 {
								...
								if instanceToAppend, ok := __gong__map_Node[identifierOfInstanceToAppend]; ok {
									instanceWhoseFieldIsAppended := __gong__map_Tree[identifier]
									if insertIndex <= len(instanceWhoseFieldIsAppended.RootNodes) {
										instanceWhoseFieldIsAppended.RootNodes = slices.Insert(instanceWhoseFieldIsAppended.RootNodes, insertIndex, instanceToAppend)
										insertIndex++ // Increment for subsequent elements in the same call
									}
								}
							}
		...
		case *ast.BasicLit, *ast.UnaryExpr, *ast.CompositeLit:
			...
			case "Node":
				...
				case "ToolTipText":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
					__gong__map_Node[identifier].ToolTipText = fielValue
				case "PreceedingIcon":
					// remove first and last char
					fielValue := basicLit.Value[1 : len(basicLit.Value)-1]
		...
		case *ast.Ident:
			...
			case "Node":
				...
				case "HasToolTip":
					...
					__gong__map_Node[identifier].HasToolTip = fielValue
				case "IsInEditMode":
					// convert string to boolean
					fielValue, err := strconv.ParseBool(ident.Name)
					if err != nil {
			...
			if Sel := selectorExpr.Sel; Sel != nil {
				...
				case "Node":
					...
					case "ToolTipPosition":
						...
						__gong__map_Node[identifier].ToolTipPosition = ToolTipPositionEnum(val)
					}
				case "SVGIcon":
					switch fieldName {
					// insertion point for selector expr assign code
