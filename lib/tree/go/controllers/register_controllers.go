...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	log.Printf("Stack github.com/fullstack-lang/gong/lib/tree/go: stack path: '%s', new ws index %d",
	...
	if backRepo == nil {
		log.Panic("Stack github.com/fullstack-lang/gong/lib/tree/go, Unkown stack", stackPath)
	}
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gong/lib/tree/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("github.com/fullstack-lang/gong/lib/tree/go:\n",
			"client no longer receiver web socket message, assuming it is no longer alive, closing websocket handler")
		fmt.Println(err)
		return
	} else {
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/tree/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/tree/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
