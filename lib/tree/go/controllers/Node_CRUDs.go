...
//
//	200: nodeDBResponse
func (controller *Controller) GetNodes(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetNodes", "Name", stackPath)
		}
	}
	...
	writeJSON(w, http.StatusOK, nodeAPIs)
}
...
//	Responses:
//	  200: nodeDBResponse
func (controller *Controller) PostNode(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostNodes", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if node != nil {
		backRepo.GetStage().AfterCreateFromFront(node)
	}
...
//
//	200: nodeDBResponse
func (controller *Controller) GetNode(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetNode", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&nodeDB, r.PathValue("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
	...
	writeJSON(w, http.StatusOK, nodeAPI)
}
...
//
//	200: nodeDBResponse
func (controller *Controller) DeleteNode(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteNode", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if nodeStaged != nil {
		backRepo.GetStage().AfterDeleteFromFront(nodeStaged, nodeDeleted)
	}
