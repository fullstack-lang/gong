...
func (controller *Controller) GetSVGIcons(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetSVGIcons", "Name", stackPath)
		}
	}
	...
	writeJSON(w, http.StatusOK, svgiconAPIs)
}
...
//	Responses:
//	  200: nodeDBResponse
func (controller *Controller) PostSVGIcon(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostSVGIcons", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if svgicon != nil {
		backRepo.GetStage().AfterCreateFromFront(svgicon)
	}
...
//
//	200: svgiconDBResponse
func (controller *Controller) GetSVGIcon(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetSVGIcon", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&svgiconDB, r.PathValue("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
	...
	writeJSON(w, http.StatusOK, svgiconAPI)
}
...
//
//	200: svgiconDBResponse
func (controller *Controller) DeleteSVGIcon(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteSVGIcon", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if svgiconStaged != nil {
		backRepo.GetStage().AfterDeleteFromFront(svgiconStaged, svgiconDeleted)
	}
