...
func (controller *Controller) UpdateSVGIcon(c *gin.Context) {
	...
	if svgiconOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), svgiconOld, svgiconNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), svgiconOld, svgiconNew, mouseEvent)
