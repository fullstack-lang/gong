...
func (controller *Controller) UpdateClassdiagram(c *gin.Context) {
	...
	if classdiagramOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), classdiagramOld, classdiagramNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), classdiagramOld, classdiagramNew, mouseEvent)
