...
func (controller *Controller) UpdateGongStructShape(c *gin.Context) {
	...
	if gongstructshapeOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), gongstructshapeOld, gongstructshapeNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), gongstructshapeOld, gongstructshapeNew, mouseEvent)
