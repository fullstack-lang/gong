...
func (controller *Controller) UpdateLinkShape(c *gin.Context) {
	...
	if linkshapeOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), linkshapeOld, linkshapeNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), linkshapeOld, linkshapeNew, mouseEvent)
