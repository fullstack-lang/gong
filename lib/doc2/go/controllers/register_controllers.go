...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	log.Printf("Stack github.com/fullstack-lang/gong/lib/doc2/go: stack path: '%s', new ws index %d",
	...
	if err != nil {
		...
		log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/doc2/go: 1st sent backRepoData of stack:", stackPath, "index", index)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format("2006-01-02 15:04:05.000000"), "github.com/fullstack-lang/gong/lib/doc2/go: sent backRepoData of stack:", stackPath, "index", index)
				}
			}
		}
	}
