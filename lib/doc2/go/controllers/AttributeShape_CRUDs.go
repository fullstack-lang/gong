...
func (controller *Controller) UpdateAttributeShape(c *gin.Context) {
	...
	if attributeshapeOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attributeshapeOld, attributeshapeNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), attributeshapeOld, attributeshapeNew, mouseEvent)
