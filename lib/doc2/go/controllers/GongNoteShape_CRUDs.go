...
func (controller *Controller) UpdateGongNoteShape(c *gin.Context) {
	...
	if gongnoteshapeOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), gongnoteshapeOld, gongnoteshapeNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), gongnoteshapeOld, gongnoteshapeNew, mouseEvent)
