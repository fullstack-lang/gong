...
func (controller *Controller) UpdateGongEnumValueShape(c *gin.Context) {
	...
	if gongenumvalueshapeOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), gongenumvalueshapeOld, gongenumvalueshapeNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), gongenumvalueshapeOld, gongenumvalueshapeNew, mouseEvent)
