...
func (stageSet *StageSet) MarshallToString(packageName string) (res string, err error) {
	...
	if stageSet.Stage != nil {
		...
		for _, cursor := range cursorOrdered {
			...
			values.WriteString(fmt.Sprintf("\n\t%s.StartX = %f", cursorIdent, cursor.StartX))
			values.WriteString(fmt.Sprintf("\n\t%s.EndX = %f", cursorIdent, cursor.EndX))
			values.WriteString(fmt.Sprintf("\n\t%s.Y1 = %f", cursorIdent, cursor.Y1))
			values.WriteString(fmt.Sprintf("\n\t%s.Y2 = %f", cursorIdent, cursor.Y2))
			values.WriteString(fmt.Sprintf("\n\t%s.DurationSeconds = %f", cursorIdent, cursor.DurationSeconds))
			values.WriteString(fmt.Sprintf("\n\t%s.Color = %s", cursorIdent, __gong__toRawStringLiteral(cursor.Color)))
			values.WriteString(fmt.Sprintf("\n\t%s.FillOpacity = %f", cursorIdent, cursor.FillOpacity))
			values.WriteString(fmt.Sprintf("\n\t%s.Stroke = %s", cursorIdent, __gong__toRawStringLiteral(cursor.Stroke)))
			values.WriteString(fmt.Sprintf("\n\t%s.StrokeOpacity = %f", cursorIdent, cursor.StrokeOpacity))
			values.WriteString(fmt.Sprintf("\n\t%s.StrokeWidth = %f", cursorIdent, cursor.StrokeWidth))
			values.WriteString(fmt.Sprintf("\n\t%s.StrokeDashArray = %s", cursorIdent, __gong__toRawStringLiteral(cursor.StrokeDashArray)))
			values.WriteString(fmt.Sprintf("\n\t%s.StrokeDashArrayWhenSelected = %s", cursorIdent, __gong__toRawStringLiteral(cursor.StrokeDashArrayWhenSelected)))
			values.WriteString(fmt.Sprintf("\n\t%s.Transform = %s", cursorIdent, __gong__toRawStringLiteral(cursor.Transform)))
			values.WriteString(fmt.Sprintf("\n\t%s.IsPlaying = %t", cursorIdent, cursor.IsPlaying))
		}
	}
...
import (
	...
	__stage_0__ "github.com/fullstack-lang/gong/lib/cursor/go/models"
)
		...
		case *ast.AssignStmt:
			...
			if node.Tok == token.ASSIGN {
				...
				case *Cursor:
					...
					case "StartX":
						inst.StartX = GongExtractFloat(rhs)
					case "EndX":
						inst.EndX = GongExtractFloat(rhs)
					case "Y1":
						inst.Y1 = GongExtractFloat(rhs)
					case "Y2":
						inst.Y2 = GongExtractFloat(rhs)
					case "DurationSeconds":
						inst.DurationSeconds = GongExtractFloat(rhs)
					case "Color":
					...
					case "IsPlaying":
						inst.IsPlaying = GongExtractBool(rhs)
					}
							}
						}
					}
