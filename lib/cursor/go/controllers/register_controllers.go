...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	log.Printf(
		"github.com/fullstack-lang/gong/lib/cursor/go: Con: '%s', index %d",
		stackPath, index,
	)

	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
		message := "WebSocket Connect, Stack github.com/fullstack-lang/gong/lib/cursor/go, Unkown stack: \"" + stackPath + "\"\n"
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gong/lib/cursor/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		...
		log.Printf(
			"github.com/fullstack-lang/gong/lib/cursor/go: %03d: '%s', index %d, size: %d bytes, hash; %s",
			refresh,
			stackPath,
			index,
