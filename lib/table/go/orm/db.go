	...
	case *CheckBoxDB:
		if existing, ok := db.checkboxDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db CheckBox github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *DisplayedColumnDB:
	...
	case *FormDivDB:
		if existing, ok := db.formdivDBs[v.ID]; ok {
			*existing = *v
			...
			return nil, errors.New("db FormDiv github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormEditAssocButtonDB:
		if existing, ok := db.formeditassocbuttonDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormEditAssocButton github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldDB:
		if existing, ok := db.formfieldDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormField github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldDateDB:
		if existing, ok := db.formfielddateDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldDate github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldDateTimeDB:
		if existing, ok := db.formfielddatetimeDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldDateTime github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldFloat64DB:
		if existing, ok := db.formfieldfloat64DBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldFloat64 github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldIntDB:
		if existing, ok := db.formfieldintDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldInt github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldSelectDB:
		if existing, ok := db.formfieldselectDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldSelect github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldStringDB:
		if existing, ok := db.formfieldstringDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldString github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormFieldTimeDB:
		if existing, ok := db.formfieldtimeDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormFieldTime github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormGroupDB:
		if existing, ok := db.formgroupDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormGroup github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *FormSortAssocButtonDB:
		if existing, ok := db.formsortassocbuttonDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db FormSortAssocButton github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *OptionDB:
		if existing, ok := db.optionDBs[v.ID]; ok {
			*existing = *v
		} else {
			return nil, errors.New("db Option github.com/fullstack-lang/gong/lib/table/go, record not found")
		}
	case *RowDB:
