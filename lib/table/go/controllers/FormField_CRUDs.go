...
func (controller *Controller) UpdateFormField(c *gin.Context) {
	...
	if formfieldOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldOld, formfieldNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldOld, formfieldNew, mouseEvent)
