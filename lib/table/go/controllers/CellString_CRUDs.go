...
func (controller *Controller) GetCellStrings(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetCellStrings", "Name", stackPath)
		}
	}
	...
	writeJSON(w, http.StatusOK, cellstringAPIs)
}
...
//	Responses:
//	  200: nodeDBResponse
func (controller *Controller) PostCellString(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostCellStrings", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if cellstring != nil {
		backRepo.GetStage().AfterCreateFromFront(cellstring)
	}
...
//
//	200: cellstringDBResponse
func (controller *Controller) GetCellString(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetCellString", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&cellstringDB, r.PathValue("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
	...
	writeJSON(w, http.StatusOK, cellstringAPI)
}
...
//
//	200: cellstringDBResponse
func (controller *Controller) DeleteCellString(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteCellString", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if cellstringStaged != nil {
		backRepo.GetStage().AfterDeleteFromFront(cellstringStaged, cellstringDeleted)
	}
