...
func (controller *Controller) UpdateCellString(c *gin.Context) {
	...
	if cellstringOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellstringOld, cellstringNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellstringOld, cellstringNew, mouseEvent)
