...
func (controller *Controller) UpdateFormDiv(c *gin.Context) {
	...
	if formdivOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formdivOld, formdivNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formdivOld, formdivNew, mouseEvent)
