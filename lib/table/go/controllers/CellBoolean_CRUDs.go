...
func (controller *Controller) UpdateCellBoolean(c *gin.Context) {
	...
	if cellbooleanOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellbooleanOld, cellbooleanNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellbooleanOld, cellbooleanNew, mouseEvent)
