...
func (controller *Controller) UpdateFormEditAssocButton(c *gin.Context) {
	...
	if formeditassocbuttonOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formeditassocbuttonOld, formeditassocbuttonNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formeditassocbuttonOld, formeditassocbuttonNew, mouseEvent)
