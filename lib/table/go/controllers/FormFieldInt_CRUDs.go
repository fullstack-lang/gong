...
func (controller *Controller) UpdateFormFieldInt(c *gin.Context) {
	...
	if formfieldintOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldintOld, formfieldintNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldintOld, formfieldintNew, mouseEvent)
