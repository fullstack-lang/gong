...
func (controller *Controller) UpdateCellFloat64(c *gin.Context) {
	...
	if cellfloat64Old != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellfloat64Old, cellfloat64New, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellfloat64Old, cellfloat64New, mouseEvent)
