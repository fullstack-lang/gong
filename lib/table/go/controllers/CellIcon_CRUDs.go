...
func (controller *Controller) UpdateCellIcon(c *gin.Context) {
	...
	if celliconOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), celliconOld, celliconNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), celliconOld, celliconNew, mouseEvent)
