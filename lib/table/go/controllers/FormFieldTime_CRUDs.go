...
func (controller *Controller) UpdateFormFieldTime(c *gin.Context) {
	...
	if formfieldtimeOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldtimeOld, formfieldtimeNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldtimeOld, formfieldtimeNew, mouseEvent)
