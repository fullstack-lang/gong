...
func (controller *Controller) UpdateFormFieldDate(c *gin.Context) {
	...
	if formfielddateOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfielddateOld, formfielddateNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfielddateOld, formfielddateNew, mouseEvent)
