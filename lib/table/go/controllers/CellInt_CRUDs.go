...
func (controller *Controller) UpdateCellInt(c *gin.Context) {
	...
	if cellintOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellintOld, cellintNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), cellintOld, cellintNew, mouseEvent)
