...
func (controller *Controller) UpdateFormSortAssocButton(c *gin.Context) {
	...
	if formsortassocbuttonOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formsortassocbuttonOld, formsortassocbuttonNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formsortassocbuttonOld, formsortassocbuttonNew, mouseEvent)
