...
func (controller *Controller) UpdateFormGroup(c *gin.Context) {
	...
	if formgroupOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formgroupOld, formgroupNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formgroupOld, formgroupNew, mouseEvent)
