...
func (controller *Controller) UpdateFormFieldSelect(c *gin.Context) {
	...
	if formfieldselectOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldselectOld, formfieldselectNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), formfieldselectOld, formfieldselectNew, mouseEvent)
