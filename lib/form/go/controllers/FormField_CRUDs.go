...
func (controller *Controller) GetFormFields(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetFormFields", "Name", stackPath)
		}
	}
	...
	writeJSON(w, http.StatusOK, formfieldAPIs)
}
...
//	Responses:
//	  200: nodeDBResponse
func (controller *Controller) PostFormField(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostFormFields", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if formfield != nil {
		backRepo.GetStage().AfterCreateFromFront(formfield)
	}
...
//
//	200: formfieldDBResponse
func (controller *Controller) GetFormField(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetFormField", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&formfieldDB, r.PathValue("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
	...
	writeJSON(w, http.StatusOK, formfieldAPI)
}
...
//
//	200: formfieldDBResponse
func (controller *Controller) DeleteFormField(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteFormField", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if formfieldStaged != nil {
		backRepo.GetStage().AfterDeleteFromFront(formfieldStaged, formfieldDeleted)
	}
