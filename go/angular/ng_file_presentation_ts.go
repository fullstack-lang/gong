...
func MultiCodeGeneratorNgPresentation(
	...
	for _, _struct := range mdlPkg.GongStructs {
		...
		for _, field := range _struct.Fields {
			...
			case *models.GongBasicField:
				...
				if field.DeclaredType == "time.Duration" {
					TSinsertions[NgPresentationTsInsertionTimeDurationFieldPerStructDeclarations] += models.Replace1(
						NgPresentationSubTemplateCode[NgPresentationTsInsertionTimeDurationFieldPerStructDeclarations],
						"{{FieldName}}", field.Name)

					TSinsertions[NgPresentationTsInsertionPerStructTimeDurationRecoveries] += models.Replace1(
		...
		modelEnums := make(map[string]*models.GongEnum)
		...
		for _, field := range _struct.Fields {
			...
			case *models.GongBasicField:
				...
				if field.GongEnum != nil {
					...
					if field.GongEnum.Type == models.Int && !isAlreadyPresent {
						map_ImportedEnums[field.GongEnum] = 0
						TSinsertions[NgPresentationTsInsertionPerStructEnumIntImports] += models.Replace1(
					...
					if field.GongEnum.Type == models.Int {
						TSinsertions[NgPresentationTsInsertionFieldPerStructEnumIntDeclarations] += models.Replace2(
							NgPresentationSubTemplateCode[NgPresentationTsInsertionFieldPerStructEnumIntDeclarations],
							"{{EnumName}}", field.GongEnum.Name,
							"{{FieldName}}", field.Name)
						TSinsertions[NgPresentationTsInsertionPerStructEnumIntRecoveries] += models.Replace2(
		...
		for _, field := range _struct.Fields {
			...
			case *models.GongBasicField:
				...
				// bais field (enum)
				if field.GongEnum != nil {
					if field.GongEnum.Type == models.String {
						subCodesHTML += models.Replace2(NgPresentationEnumHtmlSubTemplateCode[NgPresentationHtmlEnumString],
							"{{FieldName}}", field.Name,
							"{{EnumName}}", field.GongEnum.Name)
					}
					if field.GongEnum.Type == models.Int {
						subCodesHTML += models.Replace2(NgPresentationEnumHtmlSubTemplateCode[NgPresentationHtmlEnumInt],
							"{{FieldName}}", field.Name,
							"{{EnumName}}", field.GongEnum.Name)
					}
			...
			case *models.GongTimeField:
				...
				subCodesHTML +=
					models.Replace1(NgPresentationEnumHtmlSubTemplateCode[NgPresentationHtmlTimeField],
						"{{FieldName}}", field.Name)

			case *models.PointerToGongStructField:

				subCodesHTML += models.Replace3(
