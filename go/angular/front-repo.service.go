...
export class FrontRepoService {
	...
	public connectToWebSocket(Name: string): Observable<FrontRepo> {
		...
		let basePath = ` + "`" + `${protocol}//${host}/api/{{PkgPathRoot}}/v1/ws/stage` + "`" + `
...
const NgLibFrontRepoServiceTemplateUpdateOnly = `// generated code - do not edit
import { Injectable, NgZone } from '@angular/core'
import { HttpClient, HttpHeaders, HttpParams } from '@angular/common/http'
...
func CodeGeneratorNgFrontRepo(modelPkg *models.ModelPkg, skipNonUpdate bool) {
	...
	if skipNonUpdate {
		code = NgLibFrontRepoServiceTemplateUpdateOnly
	}
	...
	for structIndex, _struct := range structList {
		...
		for subTemplate := range NgFrontRepoPerStructTmplCodes {
			tmpl := NgFrontRepoPerStructTmplCodes[subTemplate]
			if skipNonUpdate && subTemplate == NgLibFrontRepoServiceImports {
				tmpl = `
import { {{Structname}}API } from './{{structname}}-api'
import { {{Structname}}, Copy{{Structname}}APITo{{Structname}} } from './{{structname}}'
`
			}
			perStructCodes[subTemplate] += models.Replace3(tmpl,
				"{{Structname}}", _struct.Name,
				"{{structname}}", structName,
