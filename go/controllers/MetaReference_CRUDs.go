...
func (controller *Controller) UpdateMetaReference(c *gin.Context) {
	...
	if metareferenceOld != nil {
		if !hasMouseEvent {
			models.OnAfterUpdateFromFront(backRepo.GetStage(), metareferenceOld, metareferenceNew, nil)
		} else {
			mouseEvent := &models.Gong__MouseEvent{
				ShiftKey: shiftKey,
			}
			models.OnAfterUpdateFromFront(backRepo.GetStage(), metareferenceOld, metareferenceNew, mouseEvent)
