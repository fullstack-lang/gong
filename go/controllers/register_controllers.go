...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	if err != nil {
		...
		log.Println(time.Now().Format(time.RFC3339Nano), "github.com/fullstack-lang/gong/go: 1st sent backRepoData of stack:", stackPath)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format(time.RFC3339Nano), "github.com/fullstack-lang/gong/go: sent backRepoData of stack:", stackPath)
				}
			}
		}
	}
