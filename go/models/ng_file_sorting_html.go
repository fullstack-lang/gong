...
const NgSortingTemplateHTML = `<div>
    <mat-form-field>
        <mat-label>Filter</mat-label>
        <input matInput (keyup)="applyFilter($event)" placeholder="Ex. ium" #input>
    </mat-form-field>
    <h1 *ngIf="dialogData">{{structname}}</h1>
    <sorting class="sorting" mat-sorting [dataSource]="matSortingDataSource" matSort>
        <!-- Checkbox Column -->
        ...
        <!-- insertion point for fields specific code -->{{` + string(rune(NgSortingHtmlInsertionColumn)) + `}}
...
<button class="sorting__save" color="primary" *ngIf="dialogData" mat-raised-button (click)="save()">
    Save
</button>`

// insertion points in the main template
type NgSortingHtmlInsertionPoint int

const (
	NgSortingHtmlInsertionColumn NgSortingHtmlInsertionPoint = iota
	NgSortingHtmlInsertionsNb
)

type NgSortingHTMLSubTemplate int

const (
	NgSortingHTMLBasicField NgSortingHTMLSubTemplate = iota
	NgSortingHTMLTimeField
	NgSortingHTMLBasicFloat64Field
	NgSortingHTMLBasicFieldTimeDuration
	NgSortingHTMLBool
	NgSortingPointerToStructHTMLFormField
	NgSortingPointerToSliceOfGongStructHTMLFormField
)

var NgSortingHTMLSubTemplateCode map[NgSortingHTMLSubTemplate]string = map[NgSortingHTMLSubTemplate]string{

	NgSortingHTMLBasicField: `
        <!-- -->
