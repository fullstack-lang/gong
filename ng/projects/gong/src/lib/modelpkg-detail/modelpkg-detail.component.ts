...
// ModelPkgDetailComponent is initilizaed from different routes
// ModelPkgDetailComponentState detail different cases 
enum ModelPkgDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class ModelPkgDetailComponent implements OnInit {
	...
	state: ModelPkgDetailComponentState = ModelPkgDetailComponentState.CREATE_INSTANCE
	...
	onChangedActivatedRoute(): void {
		...
		if (this.id == 0) {
			this.state = ModelPkgDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = ModelPkgDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getModelPkg(): void {
		...
		this.frontRepoService.pull(this.GONG__StackPath).subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case ModelPkgDetailComponentState.CREATE_INSTANCE:
						this.modelpkg = new (ModelPkgDB)
						break;
					case ModelPkgDetailComponentState.UPDATE_INSTANCE:
						let modelpkg = frontRepo.ModelPkgs.get(this.id)
						console.assert(modelpkg != undefined, "missing modelpkg with id:" + this.id)
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.ONE_MANY_ASSOCIATION_MODE) {
			...
			dialogData.ID = this.modelpkg.ID!
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "ModelPkg"
