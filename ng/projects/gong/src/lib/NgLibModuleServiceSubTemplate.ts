// generated from ng_file_enum.ts.go
export enum NgLibModuleServiceSubTemplate {
	// insertion point	
	NgLibModuleDeclarations = "NgLibModuleIndivDecls",
	NgLibModuleImports = "NgLibModuleImports",
}

export interface NgLibModuleServiceSubTemplateSelect {
	value: string;
	viewValue: string;
}

export const NgLibModuleServiceSubTemplateList: NgLibModuleServiceSubTemplateSelect[] = [ // insertion point	
	{ value: 'NgLibModuleDeclarations', viewValue: '"NgLibModuleIndivDecls"' },
	{ value: 'NgLibModuleImports', viewValue: '"NgLibModuleImports"' },
];
