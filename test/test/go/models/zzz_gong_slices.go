...
func (stage *Stage) ComputeForwardAndBackwardCommits() {
	...
	for ref := range stage.Astructs_reference {
		if _, ok := stage.Astructs[ref]; !ok {
	...
	// parse all reference instances and check if they are still staged
	for ref := range stage.AstructBstruct2Uses_reference {
		if _, ok := stage.AstructBstruct2Uses[ref]; !ok {
	...
	// parse all reference instances and check if they are still staged
	for ref := range stage.AstructBstructUses_reference {
		if _, ok := stage.AstructBstructUses[ref]; !ok {
	...
	// parse all reference instances and check if they are still staged
	for ref := range stage.Bstructs_reference {
		if _, ok := stage.Bstructs[ref]; !ok {
	...
	// parse all reference instances and check if they are still staged
	for ref := range stage.Dstructs_reference {
		if _, ok := stage.Dstructs[ref]; !ok {
	...
	// parse all reference instances and check if they are still staged
	for ref := range stage.F0123456789012345678901234567890s_reference {
		if _, ok := stage.F0123456789012345678901234567890s[ref]; !ok {
	...
	// parse all reference instances and check if they are still staged
	for ref := range stage.Gstructs_reference {
		if _, ok := stage.Gstructs[ref]; !ok {
...
func (astruct *Astruct) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", astruct.Name))
	return
}
...
func (astructbstruct2use *AstructBstruct2Use) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", astructbstruct2use.Name))
	return
}
...
func (astructbstructuse *AstructBstructUse) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", astructbstructuse.Name))
	return
}
...
func (bstruct *Bstruct) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", bstruct.Name))
	return
}
...
func (dstruct *Dstruct) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", dstruct.Name))
	return
}
...
func (f0123456789012345678901234567890 *F0123456789012345678901234567890) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", f0123456789012345678901234567890.Name))
	return
}
...
func (gstruct *Gstruct) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", fmt.Sprintf("%q", gstruct.Name))
	return
}
