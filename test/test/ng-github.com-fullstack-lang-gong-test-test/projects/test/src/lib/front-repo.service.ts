			...
			const attemptConnection = (retries: number): void => {
				...
				if (!isOfflineMode) {
					...
					socket.onerror = event => {
						console.error("github.com/fullstack-lang/gong/test/test/go WebSocket: onerror", event)
					...
					console.error("github.com/fullstack-lang/gong/test/test/go, attemptConnection: Offline mode detected, but WASM backend failed to load.")
