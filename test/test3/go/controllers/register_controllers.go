...
func registerControllers(r *gin.Engine) {
	v1 := r.Group("/api/github.com/fullstack-lang/gong/test3/go")
...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	// log.Println("Stack github.com/fullstack-lang/gong/test3/go, onWebSocketRequestForBackRepoContent")
	...
	log.Printf("Stack github.com/fullstack-lang/gong/test3/go: stack path: '%s', new ws index %d",
	...
	if backRepo == nil {
		log.Panic("Stack github.com/fullstack-lang/gong/test3/go, Unkown stack", stackPath)
	}
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gong/test3/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("github.com/fullstack-lang/gong/test3/go:\n",
			"client no longer receiver web socket message, assuming it is no longer alive, closing websocket handler")
		fmt.Println(err)
		return
	} else {
		log.Println(time.Now().Format(time.RFC3339Nano), "github.com/fullstack-lang/gong/test3/go: 1st sent backRepoData of stack:", stackPath)
	}
	for {
		select {
		case <-ctx.Done():
		...
		default:
			for nbCommitBackRepo := range updateCommitBackRepoNbChannel {
				...
				if err != nil {
					...
					log.Println(time.Now().Format(time.RFC3339Nano), "github.com/fullstack-lang/gong/test3/go: sent backRepoData of stack:", stackPath)
				}
			}
		}
	}
...
func (controller *Controller) GetLastCommitFromBackNb(c *gin.Context) {
	...
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
		log.Panic("Stack github.com/fullstack-lang/gong/test3/go/models, Unkown stack", stackPath)
	}
