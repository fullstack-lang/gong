...
func (probe *Probe) ux_tree() {
	...
	for _, gongStruct := range sliceOfGongStructsSorted {
		...
		case "A":
			...
			set := *probe.stageOfInterest.GetInstancesSet[*models.A]()
			count := 0
			for _a := range set {
		...
		case "B":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.B]()
			count := 0
			for _b := range set {
				if count >= probe.GetMaxElementsNbPerGongStructNode() {
					nodeGongstruct.Children = append(nodeGongstruct.Children, &tree_models.Node{Name: "..."})
		...
		case "C":
			nodeGongstruct.Name = name
			set := *probe.stageOfInterest.GetInstancesSet[*models.C]()
			count := 0
			for _c := range set {
				if count >= probe.GetMaxElementsNbPerGongStructNode() {
					nodeGongstruct.Children = append(nodeGongstruct.Children, &tree_models.Node{Name: "..."})
				...
				nodeInstance := &tree_models.Node{
					...
					IsNodeClickable: true,
					OnClick: func(frontNode *tree_models.Node) {
						FillUpFormFromGongstruct(_c, probe)
					},
				}
				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
