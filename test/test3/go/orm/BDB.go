...
func (backRepoB *BackRepoBStruct) GetStage() (stage *models.Stage) {
	stage = backRepoB.stage
	return
}

func (backRepoB *BackRepoBStruct) GetDB() db.DBInterface {
	return backRepoB.db
}
...
func (backRepoB *BackRepoBStruct) CommitPhaseOne(stage *models.Stage) (Error error) {
	...
	sort.Slice(bs, func(i, j int) bool {
		return stage.BMap_Staged_Order[bs[i]] < stage.BMap_Staged_Order[bs[j]]
	})
...
func (backRepoB *BackRepoBStruct) CheckoutPhaseOne() (Error error) {
	...
	bDBArray := make([]BDB, 0)
	_, err := backRepoB.db.Find(&bDBArray)
	if err != nil {
		return err
	}
...
func (backRepo *BackRepoStruct) CheckoutB(b *models.B) {
	...
	if _, ok := backRepo.BackRepoB.Map_BPtr_BDBID[b]; ok {
		...
		if id, ok := backRepo.BackRepoB.Map_BPtr_BDBID[b]; ok {
			...
			if _, err := backRepo.BackRepoB.db.First(&bDB, id); err != nil {
