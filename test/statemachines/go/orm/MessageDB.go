...
type MessagePointersEncoding struct {
	...
	// field MessageType is a pointer to another Struct (optional or 0..1)
	// This field is generated into another field to enable AS ONE association
	MessageTypeID sql.NullInt64

	// field OriginTransition is a pointer to another Struct (optional or 0..1)
	// This field is generated into another field to enable AS ONE association
	OriginTransitionID sql.NullInt64
}
...
type MessageDB struct {
	...
	// Declation for basic field messageDB.IsSelected
...
func (backRepoMessage *BackRepoMessageStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, message *models.Message) (Error error) {
	...
	if messageDB, ok := backRepoMessage.Map_MessageDBID_MessageDB[idx]; ok {
		...
		// commit pointer value message.MessageType translates to updating the message.MessageTypeID
		messageDB.MessageTypeID.Valid = true // allow for a 0 value (nil association)
		if message.MessageType != nil {
			if MessageTypeId, ok := backRepo.BackRepoMessageType.Map_MessageTypePtr_MessageTypeDBID[message.MessageType]; ok {
				messageDB.MessageTypeID.Int64 = int64(MessageTypeId)
				messageDB.MessageTypeID.Valid = true
			}
		} else {
			messageDB.MessageTypeID.Int64 = 0
			messageDB.MessageTypeID.Valid = true
		}

		// commit pointer value message.OriginTransition translates to updating the message.OriginTransitionID
		messageDB.OriginTransitionID.Valid = true // allow for a 0 value (nil association)
		if message.OriginTransition != nil {
			if OriginTransitionId, ok := backRepo.BackRepoTransition.Map_TransitionPtr_TransitionDBID[message.OriginTransition]; ok {
				messageDB.OriginTransitionID.Int64 = int64(OriginTransitionId)
				messageDB.OriginTransitionID.Valid = true
			}
		} else {
			messageDB.OriginTransitionID.Int64 = 0
			messageDB.OriginTransitionID.Valid = true
		}
...
func (messageDB *MessageDB) DecodePointers(backRepo *BackRepoStruct, message *models.Message) {
	...
	// MessageType field	
	{
		id := messageDB.MessageTypeID.Int64
		if id != 0 {
			tmp, ok := backRepo.BackRepoMessageType.Map_MessageTypeDBID_MessageTypePtr[uint(id)]

			// if the pointer id is unknown, it is not a problem, maybe the target was removed from the front
			if !ok {
				log.Println("DecodePointers: message.MessageType, unknown pointer id", id)
				message.MessageType = nil
			} else {
				// updates only if field has changed
				if message.MessageType == nil || message.MessageType != tmp {
					message.MessageType = tmp
				}
			}
		} else {
			message.MessageType = nil
	...
	// OriginTransition field	
	{
		id := messageDB.OriginTransitionID.Int64
		if id != 0 {
			tmp, ok := backRepo.BackRepoTransition.Map_TransitionDBID_TransitionPtr[uint(id)]

			// if the pointer id is unknown, it is not a problem, maybe the target was removed from the front
			if !ok {
				log.Println("DecodePointers: message.OriginTransition, unknown pointer id", id)
				message.OriginTransition = nil
			} else {
				// updates only if field has changed
				if message.OriginTransition == nil || message.OriginTransition != tmp {
					message.OriginTransition = tmp
				}
			}
		} else {
			message.OriginTransition = nil
...
func (messageDB *MessageDB) CopyBasicFieldsFromMessage(message *models.Message) {
	...
	messageDB.IsSelected_Data.Bool = message.IsSelected
	messageDB.IsSelected_Data.Valid = true
}
...
func (messageDB *MessageDB) CopyBasicFieldsToMessage(message *models.Message) {
	...
	message.Name = messageDB.Name_Data.String
	message.IsSelected = messageDB.IsSelected_Data.Bool
}
...
func (backRepoMessage *BackRepoMessageStruct) RestorePhaseTwo() {
	...
	for _, messageDB := range backRepoMessage.Map_MessageDBID_MessageDB {
		...
		// reindexing MessageType field
		if messageDB.MessageTypeID.Int64 != 0 {
			messageDB.MessageTypeID.Int64 = int64(BackRepoMessageTypeid_atBckpTime_newID[uint(messageDB.MessageTypeID.Int64)])
			messageDB.MessageTypeID.Valid = true
		}

		// reindexing OriginTransition field
		if messageDB.OriginTransitionID.Int64 != 0 {
			messageDB.OriginTransitionID.Int64 = int64(BackRepoTransitionid_atBckpTime_newID[uint(messageDB.OriginTransitionID.Int64)])
			messageDB.OriginTransitionID.Valid = true
		}
