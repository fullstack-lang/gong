...
func registerControllers(mux *http.ServeMux) {
	base := "/api/github.com/fullstack-lang/gong/test/test2/go/v1"

	mux.HandleFunc("GET " + base + "/as", GetController().GetAs)
	mux.HandleFunc("GET " + base + "/as/{id}", GetController().GetA)
	mux.HandleFunc("POST " + base + "/as", GetController().PostA)
	mux.HandleFunc("PATCH " + base + "/as/{id}", GetController().UpdateA)
	mux.HandleFunc("PUT " + base + "/as/{id}", GetController().UpdateA)
	mux.HandleFunc("DELETE " + base + "/as/{id}", GetController().DeleteA)

	mux.HandleFunc("GET " + base + "/bs", GetController().GetBs)
	mux.HandleFunc("GET " + base + "/bs/{id}", GetController().GetB)
	mux.HandleFunc("POST " + base + "/bs", GetController().PostB)
	mux.HandleFunc("PATCH " + base + "/bs/{id}", GetController().UpdateB)
	mux.HandleFunc("PUT " + base + "/bs/{id}", GetController().UpdateB)
	mux.HandleFunc("DELETE " + base + "/bs/{id}", GetController().DeleteB)
...
func (controller *Controller) onWebSocketRequestForBackRepoContent(w http.ResponseWriter, r *http.Request) {
	...
	// log.Println("Stack github.com/fullstack-lang/gong/test/test2/go, onWebSocketRequestForBackRepoContent")
	...
	if backRepo == nil {
		message := "WebSocket Connect, Stack github.com/fullstack-lang/gong/test/test2/go, Unkown stack: \"" + stackPath + "\"\n"
	...
	go func() {
		for {
			...
			if err != nil {
				log.Println("github.com/fullstack-lang/gong/test/test2/go", stackPath, "WS client disconnected:", err)
	...
	if err != nil {
		log.Println("github.com/fullstack-lang/gong/test/test2/go:\n",
		...
		parts := strings.Split("github.com/fullstack-lang/gong/test/test2/go", "/") // Assuming goFilePath holds the path
		component := "unknown"
		if len(parts) > 2 {
