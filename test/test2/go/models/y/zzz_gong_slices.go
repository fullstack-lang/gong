...
func (y *Y) GongGetUUID(stage *Stage) (uuid string) {
	...
	uuid = GenerateReproducibleUUIDv4(GetGongstructNameFromPointer(y), uint64(GetOrderPointerGongstruct(stage, y)))
	return
}
...
func (y *Y) GongMarshallIdentifier(stage *Stage) (decl string) {
	...
	decl = strings.ReplaceAll(decl, "{{GeneratedFieldNameValue}}", ToRawStringLiteral(y.Name))
	return
}
