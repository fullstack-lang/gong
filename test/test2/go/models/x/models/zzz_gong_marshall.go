...
func (stage *Stage) MarshallToString(modelsPackageName, packageName string) (res string, err error) {
	...
	sort.Slice(submodelOrdered[:], func(i, j int) bool {
		submodeli := submodelOrdered[i]
		submodelj := submodelOrdered[j]
		submodeli_order, oki := stage.SubModel_stagedOrder[submodeli]
		submodelj_order, okj := stage.SubModel_stagedOrder[submodelj]
		if !oki || !okj {
			log.Fatalln("unknown pointers")
		}
		return submodeli_order < submodelj_order
	})
	if len(submodelOrdered) > 0 {
		identifiersDecl.WriteString("\n")
	}
	for _, submodel := range submodelOrdered {

		identifiersDecl.WriteString(submodel.GongMarshallIdentifier(stage))

		initializerStatements.WriteString("\n")
		// Insertion point for basic fields value assignment
		initializerStatements.WriteString(submodel.GongMarshallField(stage, "Name"))
	}
...
func (submodel *SubModel) GongMarshallField(stage *Stage, fieldName string) (res string) {

	switch fieldName {
	case "Name":
		res = GongStringInitStatement
		res = strings.ReplaceAll(res, "{{Identifier}}", submodel.GongGetIdentifier(stage))
		res = strings.ReplaceAll(res, "{{GeneratedFieldName}}", "Name")
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", __gong__toRawStringLiteral(submodel.Name))

	default:
		log.Panicf("Unknown field %s for Gongstruct SubModel", fieldName)
	}
	return
}

// insertion point for marshall all fields methods
func (submodel *SubModel) GongMarshallAllFields(stage *Stage) (initRes string, ptrRes string) {
