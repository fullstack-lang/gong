...
func (stage *Stage) MarshallFile(filename, modelsPackageName, packageName string) {
	...
	if stage.GetGongMarshallingMode() == GongMarshallingAppendCommit {
		...
		if stage.isSquashing {
			...
			firstBrace := strings.Index(content, "func _(stage *x.Stage) {")
			if firstBrace != -1 {
				firstBrace += len("func _(stage *x.Stage) {")
...
func (x *X) GongMarshallField(stage *Stage, fieldName string) (res string) {
	...
	case "Name":
		...
		res = strings.ReplaceAll(res, "{{GeneratedFieldNameValue}}", __gong__toRawStringLiteral(x.Name))
