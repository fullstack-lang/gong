...
func (stageSet *StageSet) MarshallToString(packageName string) (res string, err error) {
	...
	if stageSet.Stage != nil {
		for _, x := range __gong__sortStageSetInstances(stageSet.Stage.Xs, stageSet.Stage.X_stagedOrder) {
			...
			xIdent := "__x" + x.GongGetIdentifier(stageSet.Stage)
			declarations.WriteString(fmt.Sprintf("\n\t%s := (&x.X{Name: %s}).Stage(stageSet.Stage)", xIdent, __gong__toRawStringLiteral(x.Name)))
	...
	ast.Inspect(inFile, func(n ast.Node) bool {
		...
		case *ast.AssignStmt:
			...
			case "x":
				switch typeName {
				case "X":
					identifierMap[ident.Name] = __gong__stageSetInit(new(X), stageSet.Stage, ident.Name, instanceName, preserveOrder)
				}
					}
				}
				return false
