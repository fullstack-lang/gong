...
func (controller *Controller) GetGstruct(c *gin.Context) {
	...
	if err := db.First(&gstructDB, c.Param("id")).Error; err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
...
func (controller *Controller) UpdateGstruct(c *gin.Context) {
	...
	// fetch the gstruct
	query := db.First(&gstructDB, c.Param("id"))
	...
	query = db.Model(&gstructDB).Updates(gstructDB)
	if query.Error != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = query.Error.Error()
