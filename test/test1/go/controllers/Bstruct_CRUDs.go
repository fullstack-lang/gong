...
func (controller *Controller) GetBstructs(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetBstructs", "Name", stackPath)
		}
	}
	...
	writeJSON(w, http.StatusOK, bstructAPIs)
}
...
//	Responses:
//	  200: nodeDBResponse
func (controller *Controller) PostBstruct(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("PostBstructs", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if bstruct != nil {
		backRepo.GetStage().AfterCreateFromFront(bstruct)
	}
...
//
//	200: bstructDBResponse
func (controller *Controller) GetBstruct(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("GetBstruct", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if _, err := db.First(&bstructDB, r.PathValue("id")); err != nil {
		var returnError GenericError
		returnError.Body.Code = http.StatusBadRequest
		returnError.Body.Message = err.Error()
		log.Println(err.Error())
	...
	writeJSON(w, http.StatusOK, bstructAPI)
}
...
//
//	200: bstructDBResponse
func (controller *Controller) DeleteBstruct(w http.ResponseWriter, r *http.Request) {
	...
	if len(_values) == 1 {
		...
		if len(value) == 1 {
			stackPath = value[0]
			// log.Println("DeleteBstruct", "Name", stackPath)
		}
	}
	backRepo := controller.Map_BackRepos[stackPath]
	if backRepo == nil {
	...
	if bstructStaged != nil {
		backRepo.GetStage().AfterDeleteFromFront(bstructStaged, bstructDeleted)
	}
