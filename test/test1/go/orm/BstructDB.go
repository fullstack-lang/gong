...
func (backRepoBstruct *BackRepoBstructStruct) GetStage() (stage *models.Stage) {
...
func (backRepoBstruct *BackRepoBstructStruct) CommitPhaseOne(stage *models.Stage) (Error error) {

	var bstructs []*models.Bstruct
	for bstruct := range stage.Bstructs {
		bstructs = append(bstructs, bstruct)
	}

	// Sort by the order stored in _stagedOrder.
	sort.Slice(bstructs, func(i, j int) bool {
		return stage.Bstruct_stagedOrder[bstructs[i]] < stage.Bstruct_stagedOrder[bstructs[j]]
	})

	for _, bstruct := range bstructs {
...
func (backRepoBstruct *BackRepoBstructStruct) CheckoutPhaseOne() (Error error) {
	...
	_, err := backRepoBstruct.db.Find(&bstructDBArray)
	if err != nil {
		return err
	}
...
func (backRepoBstruct *BackRepoBstructStruct) CheckoutPhaseTwoInstance(backRepo *BackRepoStruct, bstructDB *BstructDB) (Error error) {
	...
	bstructDB.DecodePointers(backRepo, bstruct)
...
func (backRepo *BackRepoStruct) CheckoutBstruct(bstruct *models.Bstruct) {
	...
	if _, ok := backRepo.BackRepoBstruct.Map_BstructPtr_BstructDBID[bstruct]; ok {
		...
		if id, ok := backRepo.BackRepoBstruct.Map_BstructPtr_BstructDBID[bstruct]; ok {
			...
			if _, err := backRepo.BackRepoBstruct.db.First(&bstructDB, id); err != nil {
...
func (backRepoBstruct *BackRepoBstructStruct) RestorePhaseTwo() {
	...
	for _, bstructDB := range backRepoBstruct.Map_BstructDBID_BstructDB {
		...
		db, _ := backRepoBstruct.db.Model(bstructDB)
