...
func NewBackRepo(stage *x.StageStruct, filename string) (backRepo *BackRepoStruct) {
	...
	backRepo.BackRepoA = BackRepoAStruct{
		Map_ADBID_APtr: make(map[uint]*x.A, 0),
		Map_ADBID_ADB:  make(map[uint]*ADB, 0),
		Map_APtr_ADBID: make(map[*x.A]uint, 0),

		db:    db,
		stage: stage,
	}
	backRepo.BackRepoB = BackRepoBStruct{
		Map_BDBID_BPtr: make(map[uint]*x.B, 0),
		Map_BDBID_BDB:  make(map[uint]*BDB, 0),
		Map_BPtr_BDBID: make(map[*x.B]uint, 0),
...
func (backRepo *BackRepoStruct) GetStage() (stage *x.StageStruct) {
...
func (backRepo *BackRepoStruct) Commit(stage *x.StageStruct) {
...
func (backRepo *BackRepoStruct) Checkout(stage *x.StageStruct) {
	// insertion point for per struct back repo phase one commit
...
func (backRepo *BackRepoStruct) Backup(stage *x.StageStruct, dirPath string) {
...
func (backRepo *BackRepoStruct) BackupXL(stage *x.StageStruct, dirPath string) {
	os.MkdirAll(dirPath, os.ModePerm)
...
func (backRepo *BackRepoStruct) Restore(stage *x.StageStruct, dirPath string) {
...
// Restore the database into the back repo
func (backRepo *BackRepoStruct) RestoreXL(stage *x.StageStruct, dirPath string) {
